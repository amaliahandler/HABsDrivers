...
above_100k <- PredData |>
  filter(disc_cyano == 'B4' | disc_cyano == 'B5' |  disc_cyano == 'B6')
per_100 <- round(nrow(above_100k)/lakes_num, digits = 3) * 100

above_50 <- PredData |>
  filter(disc_micx == 'M3' | disc_micx == 'M4' )
per_50 <- round(nrow(above_50)/lakes_num, digits = 3) * 100

load(paste0(here(),'/inst/states_data.rda'))

state_cols <- PredData |>
  left_join(states_data, by = 'COMID')
...
state_cols <- state_cols |>
  ...
  mutate(class_cyano = factor(case_when(
    pred_cyano_fit >= 5.0043215 ~ 'HC',
    pred_cyano_fit < 5.0043215 ~ 'LC',
    TRUE ~ 'OTHER'),
    levels = c('HC','LC'))) %>%
  arrange(class_cyano) |>
  drop_na()
