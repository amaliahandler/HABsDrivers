...
Artificial placeholder for schematic diagram so that figure numbering is correct (@fig1)
```{r}
#| label: fig1
#| fig-width: 1
#| fig-height: 1
...
We used data from the 2007, 2012, and 2017 NLA for this analysis (@nla-map)
...
In most models, the amount of cyanobacteria and probability of microcystin detection was higher in the central plains and coastal lowlands as well as the upper midwest and eastern highlands (EHIGH) compared to the western mountains (WMTNS) and xeric ecoregion (@preds-map). In addition, the cyanobacteria model with in-lake conditions had an interaction term between lake total N concentration and the ecoregion. In this instance, higher total N concentration in the EHIGH ecoregion is correlated with significantly higher cyanobacteria abundance relative to the WMTNS. There was also a small negative interaction between the total N concentration and the EHIGH, but not significant (`r int_cyano_plnlow`).
...
Residuals that are decorrelated (i.e., standardized) can be used to check assumptions in spatial models. For both models that included and excluded in-lake conditions, the decorrelated residuals did not show any obvious signs of extreme behavior suggesting lack of fit (Figure S1),  For the models of cyanobacteria abundance, spatial models outperformed independent error models with a 30% lower RMSPE and more than doubled the predicted R2 regardless of the presence of in-lake conditions (Table 2). For the probability of microcystin detection, spatial models had approximately a 10% reduction in RMSPE and a 10% increase in AUC.
...
att_cyan <- readr::read_csv(here::here("inst/model_objects/cyano_nolake_attribution.csv")) |>
  mutate(resp = "Cyano") |>
...
att_micx <- readr::read_csv(here::here("inst/model_objects/micx_nolake_attribution.csv")) |>
  mutate(resp = "Micx") |>
...
attr_fig <- ggarrange(plotlist = list(cyan_attri_plot, micx_attri_plot), ncol = 2, nrow = 1, labels = "auto")
