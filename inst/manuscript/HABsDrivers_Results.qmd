...
cy_tbox <- ggplot(n, aes(fill=cyano_ecocond, y=n, x=AG_ECO3)) + 
  ...
  geom_text(aes(label = paste0(percent, "%")), vjust = 0.5, size = 4, position = position_stack(vjust = 0.5)) +
...
ggsave(here("inst", "figures", "pred_maps_final_models.png"),
       height = 8, width = 6, units = "in", dpi = 1000)
...
box_cyano <- ggplot(PredData, aes(x = cyano_cat, y = drain_ratio, fill = cyano_cat)) +
  ...
  geom_text(
    data = cysum,
    aes(x = cyano_cat, y = upper_whisker, label = format(round(mean_drain, 0), big.mark = ",")),
    vjust = -0.5,
    hjust = "center",
    size = 3,
    fontface = "bold") +
...
box_micx <- ggplot(PredData, aes(x = micx_cat, y = drain_ratio, fill = micx_cat)) +
  ...
  geom_text(
    data = micxsum,
    aes(x = micx_cat, y = upper_whisker, label = format(round(mean_drain, 0), big.mark = ",")),
    vjust = -0.5,
    hjust = 'center',
    size = 3,
    fontface = "bold"
