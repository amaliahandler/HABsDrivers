...
#| fig-width: 6
#| fig-height: 7
#| fig-cap: "Standardized residuals for the cyanobacteria abundance (a) and probability of microcystin detection (b) models without in-lake conditions."
...
cyan_stdresid_map <- ggplot() + 
  geom_sf(data = conus, fill = "#e2e2e2", color = "white", lwd = 1) +
  geom_sf(data = filter(std_resid_comb, resp == "Cyanobacteria"), 
...
micx_stdresid_map <- ggplot() + 
  geom_sf(data = conus, fill = "#e2e2e2", color = "white", lwd = 1) +
  geom_sf(data = filter(std_resid_comb, resp == "Microcystin"), 
          aes(fill = .std.resid), pch = 21, size = 1.5) +
  colorspace::scale_fill_continuous_divergingx(palette = "BrBG", 
                                               mid = 0, 
                                               name = "Standardized\nResidual") +
...
ggarrange(plotlist = list(cyan_stdresid_map, micx_stdresid_map), ncol = 1, nrow = 2, labels = "auto")
...
#| fig-width: 6
#| fig-height: 7
#| fig-cap: "Spatial covariance for the cyanobacteria abundance (a) and probability of microcystin detection (b) models without in-lake data."
...
comp_micx <- comp_micx |>
  mutate(all_pred = factor(case_when(
    (n_dev_inputs >= 10 | p_farm_inputs >= 4) & pred_micx_fit >= 0.50 ~ 'HNHC',
    (n_dev_inputs < 10 | p_farm_inputs < 4) & pred_micx_fit >= 0.50 ~ 'LNHC',
    (n_dev_inputs >= 10 | p_farm_inputs >= 4) & pred_micx_fit < 0.50 ~ 'HNLC',
    (n_dev_inputs < 10 | p_farm_inputs < 4) & pred_micx_fit < 0.50 ~ 'LNLC',
    TRUE ~ 'OTHER'),
...
# The current analysis relies on just the nutrient inputs that are included in the respective models. How do the numbers change when all nutrient inputs included in the model are used to determine high/low nutrient inputs? What about expanding to a broader set of nutrient inputs that are not included in the model?
habs |>
  st_drop_geometry() |>
  ...
  mutate(cyano_model_inputs = ifelse(th_n_farm == "HIGH" | th_p_dev == "HIGH", "HIGH", "LOW"),
         ...
         all_model_inputs = ifelse(th_n_farm == "HIGH" | th_p_dev == "HIGH" | th_n_dev == "HIGH" | th_p_farm == "HIGH", "HIGH", "LOW")
         ) |>
  mutate(tot_n_inputs = N_Fert_Farm + N_livestock_Waste + N_Total_Deposition + N_Fert_Urban + N_Human_Waste,
         ...
         nodep_n_inputs = N_Fert_Farm + N_livestock_Waste + N_Fert_Urban + N_Human_Waste,
         nodep_p_inputs = P_f_fertilizer + P_livestock_Waste + P_nf_fertilizer + P_human_waste_kg,
         nni_no_dep_inputs = ifelse(nodep_n_inputs >= th_n | nodep_p_inputs >= th_p, "HIGH", "LOW")) |>
  pivot_longer(cols = c(cyano_model_inputs:all_model_inputs, all_nni_inputs, nni_no_dep_inputs), 
               values_to = "inputs", names_to = "input_scope") |>
  group_by(input_scope, inputs) |>
  summarize(count = n(), .groups = "drop") |>
  drop_na() |>
  mutate(includes = c(rep("N/P Farm Fert, N/P Manure, N/P Human Waste, N/P Dev Fert, N CBNF", 2),
                   rep("N/P Farm Fert, N/P Manure, N/P Human Waste, N/P Dev Fert, N/P Dep", 2),
                   rep("N Farm Fert, N Manure, N CBNF, P Dev Fert, P Human Waste", 2),
                   rep("P Farm Fert, P Manure, N Human Waste, N Dev Fert", 2),
                   rep("N/P Farm Fert, N/P Manure, N/P Human Waste, N/P Dev Fert", 2))) |>
