...
cyano_constant <- cyano_nolake[["terms"]][[2]][[2]][[3]]
lakes_num <- nrow(PredData)

# Max number of observations per lake
max_obs_per_lake <- habs |> 
  st_drop_geometry() |> 
  summarise(obs_count = n(), .by = UNIQUE_ID) |> 
  pull(obs_count) |> max()

# Proportion of microcystin observations that are detect vs. non-detect
micx_det <- habs |> filter(MICX_DET == 1) |> nrow()
micx_nd  <- habs |> filter(MICX_DET == 0) |> nrow()

perc_det <- sapply(c(micx_det, micx_nd), function(n){
  round(n/nrow(filter(habs, is.finite(MICX_DET))) * 100, 1)
})

# Find the median distance to the nearest lake
# 1. Create the full distance matrix
dist_matrix <- st_distance(habs, habs)

# 2. Create a logical matrix where TRUE indicates the rows/columns share the same UNIQUE_ID
same_id_matrix <- outer(habs$UNIQUE_ID, habs$UNIQUE_ID, "==")

# 3. Mask those distances by setting them to Inf 
# This eliminates self-matches AND different observations from the same lake
dist_matrix[same_id_matrix] <- Inf

# 4. Find the minimum distance to a *different* lake for each observation
nearest_lake_dist <- apply(dist_matrix, 1, min)

# 5. Calculate the median distance between distinct lakes
# (Optional: remove Inf if a lake has absolutely no neighbors in the dataset)
valid_distances <- nearest_lake_dist[is.finite(nearest_lake_dist)]
median_dist     <- median(valid_distances, na.rm = TRUE)
# summary(valid_distances)
...
cor_dat <- habs |>
  st_drop_geometry() |>
  mutate(np_ratio = NTL / PTL,
         p_total_inputs = p_farm_inputs + p_dev_inputs + P_Deposition) |>
  select(B_G_DENS, MICX, AMMONIA_N, DOC:NITRATE_N, PH, EVAP_INFL, np_ratio,
         TEMPERATURE, precip_mean_month, temp_mean_month, Precip8110Ws, Tmean8110Ws,
         lakemorpho_fetch, MAXDEPTH, 
         fst_ws, wet_ws, BFIWs, KffactWs,  
         N_Total_Deposition, P_Deposition, N_Total_Inputs, p_total_inputs, n_farm_inputs:p_dev_inputs, P_Legacy_P) |>
  relocate(B_G_DENS, MICX, NTL, PTL, np_ratio, NITRATE_N, AMMONIA_N, DOC, PH, TEMPERATURE, TURB, EVAP_INFL, 
           fst_ws, wet_ws, KffactWs, 
           n_farm_inputs, p_farm_inputs, n_dev_inputs, p_dev_inputs, P_Legacy_P, N_Total_Deposition, P_Deposition, N_Total_Inputs, p_total_inputs,
           precip_mean_month, temp_mean_month, Precip8110Ws, Tmean8110Ws, BFIWs, 
...
# png(filename = here("inst", "figures", "correlation_matrix.png"), 
#     width = 5000, height = 5000, res = 800)
...
est_and_sign <- fixed_df |>
  ...
  mutate(fixed_effect = c("Total N",
                          ...
                          "Ecoregion PLNLOW",
                          "Ecoregion EHIGH"),
         Type = c(rep("In-Lake Conditions", 5),
                  rep("Nutrient Inputs", 2), # starting with in-lake models only
                  rep("Lake Geometry", 2),
                  rep("Geographic Setting", 4)), .before = fixed_effect
  ) |>
...
covar_tbl <- est_and_sign |>
  ...
  bg(bg = colorer,
     ...
     part = "body") |>
  bg(j = c(5:6), i = c(1:5), bg = "#606060") |> 
  bg(j = c(3:4), i = c(6:7), bg = "#606060") |> 
  theme_box() |>
  align(align = "center", part = "header") |>
  fontsize(size = 10, part = "body") |>
  fontsize(size = 10, part = "header")
...
# ---------------------------------------------------------------------------
# 1. Full fitted values (fixed effects + spatial residual + random effects)
# ---------------------------------------------------------------------------

#' Back-transform a fitted value from model (link) scale to response scale
#'
#' @param x Numeric vector of fitted values on the model scale.
#' @param backtransform One of "log10offset" (undoes log10(y + 1000), used
#'   for the cyano models' B_G_DENS response), "logit" (undoes a logit link,
#'   returning the probability of detection for the micx models), or "none".
backtransform_fitted <- function(x, backtransform = c("log10offset", "logit", "none")) {
  backtransform <- match.arg(backtransform)
  switch(
    backtransform,
    log10offset = 10^x - 1000,
    logit       = plogis(x),
    none        = x
  )
}

#' Compute fitted values including spatial residual + random effects
#'
#' @param model A fitted spmodel object.
#' @param combine_components Logical. If TRUE (default), builds .fitted_full
#'   by summing the fixed-effects-only fitted values, the spatial dependent
#'   error term, and the random effect predictions -- use this for models
#'   like cyano_lake/cyano_nolake where augment()$.fitted is fixed-effects
#'   only. If FALSE, augment()$.fitted already includes the spatial and
#'   random effect components (e.g. the micx_lake/micx_nolake logistic
#'   mixed models), so .fitted_full is just copied from augment() directly
#'   and the spcov/randcov extraction is skipped entirely.
#' @param backtransform One of "log10offset", "logit", or "none" -- passed
#'   to backtransform_fitted() to convert .fitted_full from the model
#'   (link) scale back to the response scale. Use "log10offset" for
#'   cyano_lake/cyano_nolake (undoes log10(B_G_DENS + 1000)) and "logit"
#'   for micx_lake/micx_nolake (undoes the logit link, giving probability
#'   of detection).
#' @return A tibble with just .row_id (preserves row order for downstream
#'   joining with get_partials()), .fitted_full (model/link scale), and
#'   .fitted_full_response (back-transformed to the response scale).
get_full_fitted <- function(model, combine_components = TRUE,
                             backtransform = c("log10offset", "logit", "none")) {

  backtransform <- match.arg(backtransform)

  obdata <- model$obdata
  obdata$.row_id <- seq_len(nrow(obdata))

  if (!combine_components) {
    # .fitted already reflects fixed + spatial + random effects
    obdata$.fitted_full <- augment(model)$.fitted
    obdata$.fitted_full_response <- backtransform_fitted(obdata$.fitted_full, backtransform)
    return(select(obdata, .row_id, .fitted_full, .fitted_full_response))
  }

  # fixed-effects-only fitted values (augment()/obdata share row order)
  obdata$.fitted_fixed <- augment(model)$.fitted

  # spatial dependent-error term (also in the same row order as obdata)
  obdata$de <- as.numeric(fitted(model, type = "spcov")$de)

  # random effects: list of named vectors, one per random effect term
  # (e.g. "1 | DSGN_CYCLE", "1 | UNIQUE_ID"); vector names are the grouping
  # variable pasted onto the raw factor level (e.g. "DSGN_CYCLE2012")
  randcov <- fitted(model, type = "randcov")

  re_cols <- character(0)

  for (re_name in names(randcov)) {

    re_vec <- randcov[[re_name]]
    varname <- trimws(sub("^1\\s*\\|\\s*", "", re_name))
    out_col <- paste0("re_", varname)
    re_cols <- c(re_cols, out_col)

    raw_levels <- sub(paste0("^", varname), "", names(re_vec))

    re_df <- tibble(
      .join_key = raw_levels,
      !!out_col := as.numeric(re_vec)
    )

    obdata$.join_key <- as.character(obdata[[varname]])
    obdata <- left_join(obdata, re_df, by = ".join_key")
    obdata$.join_key <- NULL
  }

  obdata <- obdata %>%
    mutate(across(all_of(re_cols), ~ replace_na(.x, 0)))

  obdata$.fitted_full <- obdata$.fitted_fixed + obdata$de +
    rowSums(obdata[, re_cols, drop = FALSE])

  obdata$.fitted_full_response <- backtransform_fitted(obdata$.fitted_full, backtransform)

  obdata |>
    select(.row_id, .fitted_full, .fitted_full_response)
}

# ---------------------------------------------------------------------------
# 2. Y-partials for continuous fixed effects (centered + scaled)
# ---------------------------------------------------------------------------

#' Compute centered/scaled y-partials for continuous predictors
#'
#' Drops the intercept and any AG_ECO3 dummy columns, multiplies each
#' remaining predictor column by its fixed-effect coefficient, centers and
#' scales (scale() default) each partial, and returns a long/tidy tibble
#' (one row per observation-predictor pair) keyed by .row_id so it can be
#' joined back to get_full_fitted() output.
#'
#' @param model A fitted spmodel object.
#' @param drop_pattern Regex for categorical columns to drop from the
#'   partials, in addition to the intercept. Defaults to AG_ECO3.
#' @return A long tibble with columns .row_id, predictor, partial_value.
get_partials <- function(model, drop_pattern = "^AG_ECO3") {

  X <- model.matrix(model)
  coefs <- model$coefficients$fixed
  coefs <- coefs[colnames(X)]  # ensure alignment with X's column order

  partials_wide <- sweep(X, MARGIN = 2, STATS = coefs, FUN = "*") %>%
    as_tibble() %>%
    select(-`(Intercept)`, -matches(drop_pattern)) %>%
    mutate(across(everything(), ~ as.numeric(scale(.))))

  partials_wide$.row_id <- seq_len(nrow(partials_wide))

  partials_wide %>%
    pivot_longer(-.row_id, names_to = "predictor", values_to = "partial_value")
}

# ---------------------------------------------------------------------------
# 3. Chain the two together
# ---------------------------------------------------------------------------

#' Build the long/tidy dataset needed for the partial-fit graphic
#'
#' @param model A fitted spmodel object.
#' @param combine_components Passed to get_full_fitted() -- TRUE for
#'   cyano_lake/cyano_nolake, FALSE for micx_lake/micx_nolake.
#' @param backtransform Passed to get_full_fitted() -- "log10offset" for
#'   cyano_lake/cyano_nolake, "logit" for micx_lake/micx_nolake.
#' @return Long tibble: one row per observation-predictor pair, with
#'   .row_id, .fitted_full, .fitted_full_response from get_full_fitted()
#'   plus predictor and partial_value from get_partials().
build_partial_plot_data <- function(model, combine_components = TRUE,
                                     backtransform = c("log10offset", "logit", "none")) {

  backtransform <- match.arg(backtransform)

  full_df <- get_full_fitted(model, combine_components = combine_components,
                              backtransform = backtransform)
  partials_long <- get_partials(model)

  stopifnot(nrow(full_df) == n_distinct(partials_long$.row_id))

  left_join(partials_long, full_df, by = ".row_id")
}

# ---------------------------------------------------------------------------
# 4. Apply to all four models
# ---------------------------------------------------------------------------

cyano_lake_plot_data   <- build_partial_plot_data(cyano_lake,   combine_components = TRUE,
                                                    backtransform = "log10offset")
cyano_nolake_plot_data <- build_partial_plot_data(cyano_nolake, combine_components = TRUE,
                                                    backtransform = "log10offset")
micx_lake_plot_data    <- build_partial_plot_data(micx_lake,    combine_components = FALSE,
                                                    backtransform = "logit")
micx_nolake_plot_data  <- build_partial_plot_data(micx_nolake,  combine_components = FALSE,
                                                    backtransform = "logit")


# Make a map of variable names and set the order
labels_map <- tribble(~predictor, ~label,
        "NTL", "Total \nNitrogen",
        "NITRATE_N", "Nitrate",
        "DOC", "Dissolved \nOrganic \nCarbon",
        "PH", "pH",
        ...
        "EVAP_INFL", "Evaporation \nInflow \nRatio",
        "MAXDEPTH", "Lake Depth",
        "lakemorpho_fetch", "Maximum \nPotential \nFetch",
        "n_farm_inputs", "Farm N \nInputs", 
        "p_dev_inputs", "Developed P \nInputs") |>
...
cyano_ypartial_plotter <- function(plot_data){
  plot_data |>
  left_join(labels_map, by = "predictor") |>
  # clamp to the display range BEFORE it reaches the log10 color transform,
  # so negative/zero values never get passed to log10() (that's what was
  # producing the NaN/-Inf warnings)
  mutate(.fitted_capped = pmin(pmax(.fitted_full_response, 10000), 1e6)) |>
  arrange((.fitted_capped)) |>
  ggplot(aes(x = label, y = partial_value, color = .fitted_capped)) +
  geom_quasirandom(shape = 16, size = 0.75, groupOnX = TRUE) +
  scale_color_gradientn(
    colors = c("#21618C", "#5499C7", "#A9CCE3",
               "#EDBB99", "#DC7633", "#A04000"),
    name = "Fitted \nCyanobacteria \nAbundance \n(cell/mL)",
    limits = c(10000, 1e6),
    oob = squish,     # belt-and-suspenders for any floating-point edge cases
    trans = "log10",
    breaks = c(1e4, 1e5, 1e6),
    labels = c(expression(phantom(0) <= 10^4),
               expression(   10^5),
               expression(phantom(0) >= 10^6))
  ) +
...
cyano_ypartial_lake   <- cyano_ypartial_plotter(cyano_lake_plot_data)
cyano_ypartial_nolake <- cyano_ypartial_plotter(cyano_nolake_plot_data)

micx_ypartial_plotter <- function(plot_data){
  plot_data |>
  left_join(labels_map, by = "predictor") |>
  mutate(.fitted_full_response = .fitted_full_response * 100) |>
  arrange((.fitted_full_response)) |>
  ggplot(aes(x = label, y = partial_value, color = .fitted_full_response)) +
  geom_quasirandom(shape = 16, size = 0.75, groupOnX = TRUE) +
...
micx_ypartial_lake   <- micx_ypartial_plotter(micx_lake_plot_data)
micx_ypartial_nolake <- micx_ypartial_plotter(micx_nolake_plot_data)

# Compiled into one massive figure panel
ggarrange(plotlist = list(cyano_ypartial_lake,
                          micx_ypartial_lake,
                          cyano_ypartial_nolake,
                          micx_ypartial_nolake), 
          ncol = 2, nrow = 2, labels = "auto")

ggsave(here("inst", "figures", "y_partial_panel.png"), height = 6, width = 10, dpi = 600)
...
# ggsave(here("inst", "figures", "spatial_resid_maps_nolake_models.png"),
#        height = 7, width = 6, units = "in", dpi = 1000)

```

```{r generate disc cyano}
...
PredData <- PredData |>
  mutate(disc_cyano = factor(case_when(pred_cyano_fit < 4.41497 ~ 'B1', # under 25k
                                       ...
                                       pred_cyano_fit >= 4.70757 & pred_cyano_fit < 5.0043215 ~ 'B3', # 50k - 100k
                                       pred_cyano_fit >= 5.0043215 & pred_cyano_fit < 5.39967 ~ 'B4', # 100k - 250k
...
```{r state-level summaries}

# load(paste0(here(),'/inst/PredDataMini.rda'))

cyano_pct <- PredData |>
  group_by(state) |>
  count(cyano_ecocond) |>
  mutate(percentage = (n / sum(n)) * 100) |>
  filter(cyano_ecocond == 'High')

state_pull <- function(state_inp){
  cyano_pct |>
  filter(state == paste(state_inp)) |>
  pull(percentage) |>
  round(digit = 1)
}
...
pred_pct <- PredData |>
  ...
  count(micx_ecocond) |>
  mutate(percentage = (n / sum(n)) * 100) |>
  filter(micx_ecocond == 'Above')

micx_pull <- function(state_inp){
  pred_pct |>
  filter(state == paste(state_inp)) |>
  pull(percentage) |>
  round(digit = 1)
...
per_100 <- round(nrow(filter(PredData, cyano_transform_fit >= 100000)) / nrow(PredData) * 100, 1)
per_50  <- round(nrow(filter(PredData, pred_micx_fit >= 0.5)) / nrow(PredData) * 100, 1)

```

### Predicted CyanoHABs

Across the CONUS, `r per_100`% of lakes were predicted to have ≥100,000 cells/mL cyanobacteria abundance (@fig-preds-map). Illinois had the highest proportion of high cyanobacteria risk lakes, with `r state_pull('IL')`% of the lakes in the dataset predicted to be above 100,000 cells/mL. Texas and South Dakota have `r state_pull('TX')`% and `r state_pull('SD')`% of lakes included in the dataset predicted to be above 100,000 cells/mL, respectively. In contrast, just `r per_50`% of lakes across the CONUS had a ≥50% probability of microcystin detection. High microcystin probability was concentrated in North and South Dakota, Minnesota, Iowa, and Illinois. North Dakota had the highest proportion of lakes in the dataset above 50% probability of microcystin detection, with `r micx_pull('ND')`%. Illinois and Iowa followed with `r micx_pull('IL')`% and `r micx_pull('IA')`% of lakes above 50% probability of microcystin, respectively.

```{r predictions maps}
#| label: fig-preds-map
#| fig-width: 6
#| fig-height: 8
#| fig-cap: !expr glue::glue("Predicted cyanobacteria abundance (a) and probability of microcystin detection at or above 0.1 µg/L (b) in {prettyNum(lakes_num, big.mark = ',')} on-network lakes. Lakes ≥100,000 cells/mL for cyanobacteria abundance or ≥50% probability of microcystin detection shown in warmer colors and lakes below these thresholds are shown in cooler colors.")
...
cyano_pred_map <- ggplot() +
  ...
  geom_sf(data = sub_25,
          ...
          size = 0.3,
          alpha = 0.8)  +
  geom_sf(data = conus, fill = NA, color = "black", lwd = 0.1) +
...
# ggsave(here("inst", "figures", "pred_maps_final_models.png"),
#        height = 7, width = 6, units = "in", dpi = 1000)

```


```{r predictions by ecoregion thresholds bar chart}
#| label: fig-pred-risk-barchart
#| fig-width: 8
#| fig-height: 4
#| fig-cap: "Number and proportion of lakes at high versus low risk of cyanobacteria abundance (left) or probability of microcystin detection (right) relative to ecoregional thresholds determined based on survey data for the Eastern Highlands (EHIGH), Central and Coastal Plains (PLNLOW), and Western Mountains (WMTNS). "
...
w_n <- n |>
  subset(AG_ECO3 == "WMTNS" & cyano_ecocond == "High")

cy_tbox <- ggplot(n, aes(fill=cyano_ecocond, y=n, x=AG_ECO3)) + 
  geom_bar(position="stack", stat="identity") +
  geom_text(data = subset(n, AG_ECO3 == "EHIGH" | AG_ECO3 == "PLNLOW" | AG_ECO3 == "WMTNS" & cyano_ecocond == "Low"),
            aes(label = paste0(sprintf("%.1f", percent), "%")), 
            vjust = 0.5, 
            size = perc_size, 
            position = position_stack(vjust = 0.5)) +
  geom_text(data = w_n, 
            aes(label = paste0(sprintf("%.1f", percent), "%"), y = 22000),
            size = perc_size) +
  scale_fill_manual(values = c('High' = "#9c008299",'Low' = "#4e856299")) +
  coord_flip() +
  labs(title = 'Cyanobacteria', y = NULL, x = NULL, fill = 'Threshold category') +
  theme_bw() +
...
w_m <- m |>
  subset(AG_ECO3 == "WMTNS" & micx_ecocond == "Above")

m_tbox <- ggplot(m, aes(fill=micx_ecocond, y=n, x=AG_ECO3)) + 
  geom_bar(position="stack", stat="identity") +
  geom_text(data = subset(m, AG_ECO3 == "EHIGH" | AG_ECO3 == "PLNLOW" | AG_ECO3 == "WMTNS" & micx_ecocond == "Below"),
            aes(label = paste0(sprintf("%.1f", percent), "%")),
            vjust = 0.5, 
            size = perc_size, 
            position = position_stack(vjust = 0.5)) +
  geom_text(data = w_m, 
            aes(label = paste0(sprintf("%.1f", percent), "%"), y = 20000),
            size = perc_size) +
  scale_fill_manual(values = c('Above' = "#9c008299",'Below' = "#4e856299")) +
  coord_flip() +
  labs(title = 'Microcystin', y = NULL, x = NULL) +
  theme_bw() +
  theme(axis.text.y = element_blank(),
        axis.text.x = element_text(size = 11))

fig <- ggarrange(plotlist = list(cy_tbox, m_tbox), ncol = 2, nrow = 1, 
                 align = "h",
                 common.legend = TRUE)

annotate_figure(fig,
                bottom = text_grob("Number of lakes", color = "black", size = 14))

# ggsave("inst/figures/thresholds_boxplot.tiff", width = 8, height = 4, units = "in", dpi = 600)
...
comp_cyano <- PredData |>
  ...
  mutate(
    ws_nut_inputs = ifelse((n_farm_inputs + n_dev_inputs) >= 10 | (p_farm_inputs + p_dev_inputs) >= 4, "HIGH", "LOW"),
    all_pred = factor(case_when(
      ws_nut_inputs == "HIGH" & pred_cyano_fit >= 5.0043215 ~ 'HNHC',
      ws_nut_inputs == "LOW" & pred_cyano_fit >= 5.0043215 ~ 'LNHC',
      ws_nut_inputs == "HIGH" & pred_cyano_fit < 5.0043215 ~ 'HNLC',
      ws_nut_inputs == "LOW" & pred_cyano_fit < 5.0043215 ~ 'LNLC',
      TRUE ~ 'OTHER'),
      levels = c('HNHC','HNLC','LNHC','LNLC'))
    ) |>
    arrange(all_pred)

cyano_nutr <- round((table(comp_cyano$all_pred)/lakes_num)*100, digits = 1)
expec_cyano <- cyano_nutr[['HNHC']] + cyano_nutr[['LNLC']]
off_cyano <- cyano_nutr[['HNLC']] + cyano_nutr[['LNHC']]

micx_nutr <- round((table(comp_micx$all_pred)/lakes_num)*100, digits = 1)
expec_micx <- micx_nutr[['HNHC']] + micx_nutr[['LNLC']]
off_micx <-  micx_nutr[['HNLC']] + micx_nutr[['LNHC']]

```

The farm and developed nutrient inputs included in the models were lower in the West and Eastern Highland ecoregions, while much of the Plains and Lowland ecoregion had higher inputs (@fig-nut-input-map). The majority of the cyanoHABs predictions (`r expec_cyano`% for cyanobacteria, `r expec_micx`% for microcystin) were consistent with expectations based on nutrient inputs included in the model, meaning that both cyanoHABs predictions and nutrient inputs were in the same cutoff category (Table 3). Lakes with either lower nutrient inputs and high cyanoHABs risk or higher nutrient inputs and low cyanoHABs risk accounted for `r off_cyano`% of cyanobacteria predictions and `r off_micx`% of microcystin predictions.

```{r table nutrient/cyanoHABs categories, eval = FALSE}
# The barplots are being used instead of this table
...
```{r map nutrient/cyanoHABs categories}
#| label: fig-nut-risk-maps
#| fig-width: 6
#| fig-height: 8
#| fig-cap: "Lake predictions symbolized by both farm and developed landscape watershed nutrient inputs as high (HN; >10 kg N/ha/yr or >4 kg P/ha/yr) or low (LN) and either high (HC) or low (LC) risk for cyanobacteria abundance (a) or probability of microcystin detections (b)."
...
nurisk_c_map <- ggplot(PredData, aes(color = cyano_cat)) +
  geom_sf(size = 0.3,
          alpha = 0.8) +
...
nurisk_m_map <- ggplot(PredData, aes(color = micx_cat)) +
  geom_sf(size = 0.3,
          alpha = 0.8) +
...
nurisk_map_comb <- ggarrange(plotlist = list(nurisk_c_map, nurisk_m_map), ncol = 1, nrow = 2, labels = "auto", align = "h", common.legend = TRUE, legend = "bottom")

# ggsave(here("inst", "figures", "nutrient_cyanoHAB_risk_map.png"), 
#        plot = nurisk_map_comb, 
#        width = 6, height = 8, units = "in", dpi = 800)
...
```{r nutrient input comparisons, include = FALSE, eval = FALSE}
# Note that this table is no longer needed now that the models have the same set of covariates.
...
```{r drainage ratio table, message = FALSE, eval = FALSE}
#| label: fig-drainage-table
#| fig-width: 6
#| fig-height: 4
#| fig-cap: "Watershed drainage ratio mean and median statistics shown for each nutrient / risk cyanoHABs category across ecoregions for microcystin and cyanobacteria."

# Note that this table is not being included. Instead the results are shown graphically with boxplots.

drain_stats <- readRDS(paste0(here(), "/inst/drain_stats_all.rds"))
...
drain_stats |>
  ...
  add_header_row(
    values = c("", "Cyanobacteria", "Microcystin"), 
    colwidths = c(1, 3, 3)) |>
  merge_v(j = ~ ag_eco3) |>
  theme_booktabs(bold_header = TRUE) |>
  add_header_lines("Nutrient Risk Categories and Watershed Drainage Ratio") |>
    theme_box() |>
  align(align = "center", part = "header") |>
  fontsize(size = 10, part = "body") |>
  fontsize(size = 10, part = "header") |>
  colformat_double(digits = 1) |>
  bold(i = c(3, 7, 11), j = c(3, 6)) |>
  bold(i = c(3, 7, 11), j = c(4, 7)) |>
  autofit()
...
```{r drainage ratio boxplot, message = FALSE}
#| label: fig-drainage-boxplot
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Boxplot of watershed drainage ratio across ecoregions for each nutrient/ risk category."
...
# Q1 <- quantile(PredData$drain_ratio, 0.25)
# Q3 <- quantile(PredData$drain_ratio, 0.75)
# IQR_val <- IQR(PredData$drain_ratio)
# 
# drain_clean 
...
box_cyano <- ggplot(PredData, aes(x = cyano_cat, y = drain_ratio, fill = cyano_cat)) +
  stat_boxplot(geom = "errorbar", width = 0.2, color = "#797979") +
  geom_boxplot(outlier.shape = NA, color = "#797979", alpha = 0.8) + 
  geom_text(
    data = cysum,
    aes(y = 1000, x = cyano_cat, label = mean_drain),
    size = 3,
    fontface = "bold") +
  facet_wrap(~AG_ECO3) +
  scale_fill_manual(values = nurisk) +
  coord_cartesian(ylim = c(0, 1000)) +
  labs(x = NULL, y = "Drainage Ratio") +
  theme_bw() +
  theme(axis.text.x = element_text(size = 11),
        axis.text.y = element_text(size = 11)) +
  guides(fill = "none") 
...
box_micx <- ggplot(PredData, aes(x = micx_cat, y = drain_ratio, fill = micx_cat)) +
  ...
  geom_text(
    data = micxsum,
    aes(y = 1000, x = micx_cat, label = mean_drain),
    size = 3,
    fontface = "bold") +
  facet_wrap(~AG_ECO3) +
...
ggarrange(plotlist = list(box_cyano, box_micx), ncol = 1, nrow = 2)

# ggsave(here("inst", "figures", "drainage_ratio_boxplot.png"), width = 8, height = 6, units = "in", dpi = 600)
...
- Tables S1-S8
- Figures S1-S4 \*\* @fig-covar-contri \*\* @fig-std-resid-maps \*\* @fig-resid-maps-inlake \*\* @fig-nut-input-map
...
habs <- habs |>
  mutate(np_ratio = NTL/PTL, .after = PTL) |>
  mutate(p_total_inputs = p_farm_inputs + p_dev_inputs + P_Deposition, .after = P_nf_fertilizer) |>
  mutate(P_Legacy_P = ifelse(P_Legacy_P < 0, NA, P_Legacy_P))
...
vec_list <- list(
  ...
  c("np_ratio", "Nitrogen/Phosphorus Ratio", "unitless", "Lake", "NLA"),
  ...
  c("TEMPERATURE", "Water Temperature", "degC", "Lake", "NLA"),
  ...
  c("N_Fert_Farm", "Farm Fertilizer N", "kg/ha/yr", "Watershed", "NNI"),
  c("N_livestock_Waste", "Livestock Waste N", "kg/ha/yr", "Watershed", "NNI"),
  c("N_CBNF", "Cultivated Crop Biological Nitrogen Fixation", "kg/ha/yr", "Watershed", "NNI"),
  c("P_f_fertilizer", "Farm Fertilizer P", "kg/ha/yr", "Watershed", "NNI"),
  c("P_livestock_Waste", "Livestock Waste P", "kg/ha/yr", "Watershed", "NNI"),
  c("N_Human_Waste", "Human Waste N", "kg/ha/yr", "Watershed", "NNI"),
  c("N_Fert_Urban", "Non-Farm Fertilizer N", "kg/ha/yr", "Watershed", "NNI"),
  c("P_human_waste_kg", "Human Waste P", "kg/ha/yr", "Watershed", "NNI"),
  c("P_nf_fertilizer", "Non-Farm Fertilizer P", "kg/ha/yr", "Watershed", "NNI"),
  c("n_farm_inputs", "Total Farm N Inputs", "kg/ha/yr", "Watershed", "NNI"),
  c("p_farm_inputs", "Total Farm P Inputs", "kg/ha/yr", "Watershed", "NNI"),
  c("n_dev_inputs", "Total Developed N Inputs", "kg/ha/yr", "Watershed", "NNI"),
  c("p_dev_inputs", "Total Developed P Inputs", "kg/ha/yr", "Watershed", "NNI"),
  c("P_Legacy_P", "Legacy P Inputs", "kg/ha/yr", "Watershed", "NNI"),
  c("N_Total_Deposition", "Total N Deposition", "kg/ha/yr", "Watershed", "NNI"),
  c("P_Deposition", "Total P Deposition", "kg/ha/yr", "Watershed", "NNI"),
  c("N_Total_Inputs", "Total N Inputs", "kg/ha/yr", "Watershed", "NNI"),
  c("p_total_inputs", "Total P Inputs", "kg/ha/yr", "Watershed", "NNI"),
  c("fst_ws", "Forested Land Cover", "percent", "Watershed", "NLCD 2011"),
  c("wet_ws", "Wetland Land Cover", "percent", "Watershed", "NLCD 2011"),
  c("KffactWs", "Soil Erodability Factor", "unitless", "Watershed", "LakeCat"),
  ...
  c("Tmean8110Ws", "Mean Annual Temperature", "degC", "Watershed", "LakeCat")
)

summary_covariate_table <- do.call(rbind, lapply(vec_list, summerizer)) |>
  mutate(Type = c(rep("In-Lake Conditions", 12), 
                  rep("Lake Geometry", 2), 
                  rep("Nutrient Inputs", 21), 
...
covar_labs <-  c("NTL" = "Total N",
                 ...
                 "N_Total_Inputs" = "Total N Inputs",
                 "p_total_inputs" = "Total P Inputs",
                 "KffactWs" = "Soil Erodability",
                 "fst_ws" = "Forest Cover",
...
# # Save the tables for the new model comparison markdown
saveRDS(mod_comp_tabs, here("inst", "model_objects", "model_tables_update.rds"))
...
Table S5. `r mod_comp_tabs[[4]][[1]]` `r mod_comp_tabs[[4]][[2]]`

\pagebreak

Table S6. Final model fixed effect coefficient estimates and standard errors with p-values (ns \>0.05, \* \<0.05, \*\* \<0.01, \*\*\*\* \< 0.001) for cyanobacteria (Cyano) and microcystin (Micx) models. Models where in-lake data were present or absent are included. Grey cells are covariates that were excluded from model development. Empty cells are covariates that were not included in the final model. Brighter colors denote a lower p-value for either positive (pink) or negative (blue) relationships. Note the scale and range of the covariates in Table S1.
...
est_and_sign <- fixed_df |>
  ...
  mutate(fixed_effect = c("Total N",
                          ...
                          "Developed P Inputs",
                          "Lake Depth",
                          "Maximum Potential Fetch",
                          "Annual Precip",
                          "Annual Air Temp",
                          "Central Plains & Coastal Lowlands",
                          "Eastern Highlands"),
         Type = c(rep("In-Lake Conditions", 5),
                  rep("Nutrient Inputs", 2), # starting with in-lake models only
                  rep("Lake Geometry", 2),
...
# # Save the tables for the new model comparison markdown
saveRDS(covar_tbl, here("inst", "model_objects", "model_full_covariate_update.rds"))
```
...
mu <- "\u00b5"  # micro sign, for "µg/L"

habs_binned <- habs |>
  mutate(
    MICX_bin = case_when(
      is.na(MICX)          ~ "Non-Detect (<0.1)",
      MICX <= 1             ~ "0.1-1",
      MICX <= 10            ~ ">1-10",
      MICX <= 25             ~ ">10-25",
      TRUE                  ~ ">25"
    ...
    MICX_bin = factor(
      MICX_bin,
      levels = c(paste0("Non-Detect (<0.1)"),
                 "0.1-1", ">1-10", ">10-25", ">25")
    )
  )

detect_colors <- c("#FED976", "#FD8D3C", "#E31A1C", "#800026")
bin_colors <- setNames(
  c("grey60", detect_colors),
  levels(habs_binned$MICX_bin)
)

habs_binned |>
  arrange(MICX_bin) |>  # plot non-detects first, detections on top
  ggplot() +
  geom_point(aes(x = DOC, y = B_G_DENS, color = MICX_bin)) +
  scale_x_log10() +
  scale_y_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  labs(y = "Cyanobacteria Abundance (cells/mL)", x = "Dissolved Organic Carbon (mg/L)") +
  scale_color_manual(
    values = bin_colors,
    name = paste0("Microcystin (", mu, "g/L)")
  ) +
  theme_bw()
