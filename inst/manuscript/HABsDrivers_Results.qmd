...
micx_lake_loocv <- readRDS(here("inst", "model_objects", "model_micx_withlakedata_loocv.rds"))

micx_nolake_loocv <- readRDS(here("inst", "model_objects", "model_micx_nolakedata_loocv.rds"))

# Make a map of variable names and set the order
labels_map <- tribble(~variable, ~label,
        "NTL", "Total Nitrogen",
        "NITRATE_N", "Nitrate",
        ...
        "MAXDEPTH", "Lake Depth",
        "lakemorpho_fetch", "Maximum Potential Fetch",
        "n_farm_inputs", "Farm N Inputs", 
        "p_dev_inputs", "Developed P Inputs",
        "AG_ECO3PLNLOW", "Central Plains & Lowlands",
        "AG_ECO3EHIGH", "Eastern Highlands",
        "spde", "Spatial Residual") |>
...
aug_df <- micx_lake |>
  augment() |>
  mutate(
    spde = fitted(micx_lake, type = "spcov")$de,
    .fitted = micx_lake_loocv$cv_predict,
    resp = "Microsystin",
    model = "Lake"
  ) |>
  select(resp, model, spde, .fitted) |>
  st_drop_geometry()

# Extract augmented data and model coefficients
aug_df <- cyano_lake |>
  augment() |>
...
# 2. Extract the model matrix to naturally handle the AG_ECO3 dummy variables
# This creates a matrix of your raw data exactly as the model saw it
X <- model.matrix(cyano_lake)

# NOTE TO REMOVE CATEGORICAL and SPDE from these figures
# 3. Create a named vector of the coefficients
coefs <- cyano_lake$coefficients$fixed

# 4. Calculate the partial values (Covariate Value * Coefficient)
# sweep() multiplies each column in X by the matching value in the coefs vector
partials <- sweep(X, MARGIN = 2, STATS = coefs, FUN = "*") |>
  as_tibble() |> 
  # Drop the intercept 
  select(-`(Intercept)`) |> 
  # Center and standardize each column (scale() does exactly this)
  # as.numeric() is used to prevent scale from creating nested 1D matrices
  mutate(across(everything(), ~ as.numeric(scale(.)))) |> 
  # Bind these new columns back to the original augmented dataframe
  bind_cols(aug_df)

att_cyan <- partials |>
  pivot_longer(c(NTL:AG_ECO3EHIGH, spde), names_to = "variable", values_to = "variable_contr") |>
  left_join(labels_map, by = "variable") |>
  arrange(.fitted)
...
cyan_attri_plot <- ggplot(att_cyan, aes(x = label, y = variable_contr, color = .fitted)) +
  geom_quasirandom(shape = 16, size = 0.75, groupOnX = TRUE) +
  scale_color_gradientn(colors = c("#21618C","#5499C7","#A9CCE3",
                                   "#EDBB99","#DC7633","#A04000"),
                        name = "Fitted \nCyanobacteria \nAbundance \n(cell/mL)" # ,
                        # breaks = c(log10(3000+1000), log10(10000+1000),
                        #            log10(30000+1000), log10(100000+1000)),
...
ggarrange(plotlist = list(cyan_attri_plot, micx_attri_plot), ncol = 1, nrow = 2, labels = "auto")
