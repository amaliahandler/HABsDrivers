...
# 1. Extract your augmented data and tidy coefficients
aug_df <- augment(cyano_lake) |>
  select(.fitted) |>
  st_drop_geometry()
coef_df <- tidy(cyano_lake)
...
partials <- sweep(X, MARGIN = 2, STATS = coefs, FUN = "*") |>
  as_tibble() |> 
  # Drop the intercept (it's a constant, not a covariate contribution)
  select(-`(Intercept)`) |> 
  ...
  # Add the 'ypart_' prefix to all columns
  # rename_with(~ paste0("ypart_", .)) |> 
  # Bind these new columns back to your original augmented dataframe
  bind_cols(aug_df)
