...
Artificial placeholder for schematic diagram so that figure numbering is correct (@fig-holder)
```{r}
#| label: fig-holder
#| fig-width: 1
#| fig-height: 1
#| fig-cap: "Placeholder for schematic diagram"

ggplot(habs)
```

We used data from the 2007, 2012, and 2017 NLA for this analysis (@fig-nla-map)
...
nla_map_cyan <- ecor_map +
  new_scale_fill() +
  geom_sf(data = nla_vis, shape = 24, aes(fill = disc_cyano), size = 1, stroke = 0.2) +
...
nla_map_micx <- ecor_map + 
  new_scale_fill() +
  geom_sf(data = nla_vis_micx, shape = 24, aes(fill = MICX_DET), size = 1, stroke = 0.2) +
...
#| fig-width: 6.5
#| fig-height: 6.5
#| fig-cap: "Pairwise spearman's rank correlation matrix of all response variables and continuous covariates considered for the models. Blue represent positive correlations and red represent negative correlations. Darker colors and tighter ellipses represent larger correlation coefficients and stronger relationships. See Table S1 for summaries of variable ranges and sources."
...
In most models, the amount of cyanobacteria and probability of microcystin detection was higher in the central plains and coastal lowlands as well as the upper midwest and eastern highlands (EHIGH) compared to the western mountains (WMTNS) and xeric ecoregion. In addition, the cyanobacteria model with in-lake conditions had an interaction term between lake total N concentration and the ecoregion. In this instance, higher total N concentration in the EHIGH ecoregion is correlated with significantly higher cyanobacteria abundance relative to the WMTNS. There was also a small negative interaction between the total N concentration and the EHIGH, but not significant (`r int_cyano_plnlow`).

#### Spatial Influence and Model Fit

Residuals that are decorrelated (i.e., standardized) can be used to check assumptions in spatial models. For both models that included and excluded in-lake conditions, the decorrelated residuals did not show any obvious signs of extreme behavior suggesting lack of fit (Figure S2),  For the models of cyanobacteria abundance, spatial models outperformed independent error models with a 30% lower RMSPE and more than doubled the predicted R2 regardless of the presence of in-lake conditions (Table 2). For the probability of microcystin detection, spatial models had approximately a 10% reduction in RMSPE and a 10% increase in AUC.
...
A benefit of incorporating spatial autocorrelation directly into modeling is that it allows for the examination of a spatial version of model errors (i.e., residuals) interpretable on the same scale as the covariates (Dumelle et al. 2023). Visualizing these model errors spatially helps identify the areas in which the spatial model accounted for autocorrelation in either the positive or negative direction (@fig-spcov-maps; Figure S3). For example, in the cyanobacteria model, the spatial errors are strongly positive in portions of the northern central plains, Midwestern states, and mid-Atlantic regions of the country, suggesting that cyanobacteria in these regions was higher than expected under a nonspatial model. In contrast, the spatial errors are strongly negative in the Northeastern part of the country. The microcystin model spatial random errors exhibited similar behavior as for the cyanobacteria model, being strongly positive in the Midwestern and central northern plains regions with pockets in Utah, Arizona, Texas, and Florida, while being strongly negative in the Northeastern part of the country. We revisit these ideas in the Discussion, using the spatial errors to motivate drivers that may be influencing cyanobacteria and microcystin on a local rather than the national scale. 

```{r spatial covariance maps for no-lake models}
#| label: fig-spcov-maps
#| fig-width: 6
#| fig-height: 7
#| fig-cap: "Spatial residuals for the cyanobacteria abundance (a) and probability of microcystin detection (b) models with in-lake conditions absent. See Figure S3 for the models where in-lake conditions are present"
...
Across the conterminous United States, 62.2% of lakes were at high risk, having above 100,000 cells/mL for predicted cyanobacteria abundance (@fig-preds-map). Illinois had the highest proportion of high cyanobacteria risk lakes, with 100% of the lakes in the dataset predicted to be above 100,000 cells/mL. Texas and South Dakota have 99.6% and 98.3% of their lakes in our dataset at high cyanobacteria risk, respectively. Areas with high proportion of at-risk lakes were in landscapes dominated by agriculture and high land development, specifically the central plains region of the US. In contrast, based on the microcystin presence/absence model, just 14.8% of lakes had a 50% or higher probability of microcystin detection. High microcystin risk was more geographically concentrated and less widespread than cyanobacteria. High microcystin probability was concentrated in the north central region around North and South Dakota, with notable hotspots in the southeast. North Dakota had the highest proportion of lakes in our dataset above this microcystin detection threshold, with 96.3%. Illinois and Iowa followed with 71.1% and 68.5% high microcystin risk, respectively.

```{r predictions maps}
#| label: fig-preds-map
#| fig-width: 6
#| fig-height: 8
#| fig-cap: "Predicted cyanobacteria abundance (a) and probabilty of microcystin detection at or above 0.1 ug/L (b) in 124,529 on-network lakes. Lakes ≥100,000 cells/mL for cyanobacteria abundance or 50% probability of microcystin detection shown in warmer colors and lakes below these thresholds are shown in cooler colors."
...
We investigated the major drivers of cyanoHABs across the county, with further analysis focused on the relationship between anthropogenic nutrients inputs and risk of cyanoHABs (Figure S4). With the added layer of watershed nutrient inputs, we found that 61.6% of our cyanobacteria predictions and 72.3% of our microcystin predictions were correlated with watershed anthropogenic nutrient inputs. This represents the lakes that fall into the high categories for both cyanoHABs risk and landscape nutrient inputs Cyanobacteria predictions had a higher proportion of lakes in the high nutrient and correspondingly high cyanoHABs category than the microcystin predictions. The ‘high-high’ category accounted for 28.7% of the lakes in the cyanobacteria predictions, and only 8% in the microcystin predictions. We paid special attention to lakes with an inverse relationship between nutrient inputs and cyanoHABs risk, those with high watershed nutrients inputs and low cyanoHABs risk and those with low nutrient inputs and high cyanoHABs risk. 33.5% of the lakes in our dataset had low watershed nutrient inputs, but high cyanoHABs risk of cyanobacteria. This pattern was observed in 6.5% of the lakes in our dataset for microcystin. The inverse pattern was observed for lakes with high nutrient inputs but correspondingly low cyanoHABs risk, with more lakes in the microcystin predictions (21.1%) falling into this category than cyanobacteria (4.9%). Within this nutrient-cyanoHABs risk framework, we explored three variables and their varying distributions in the cyanobacteria and microcystin predictions. Baseflow index, precipitation, and area:depth ratio all had significant differences of means within the nutrient- cyanoHABs risk groups, with p-values below 0.001 when analyzed with an ANOVA statistical test (@fig-density-plots).  

The relationship between the area:depth ratio and the nutrient-cyanoHABs risk categories were stronger for the cyanobacteria predications than the microcystin. Regardless of nutrient inputs, whether high or low, lakes with higher median area:depth ratio (larger surface area when compared to their depth) were correlated with high cyanobacteria risk. For both the microcystin and cyanobacteria predictions, higher proportion of baseflow in the system was associated with lower risk, even with high or low nutrient inputs. In low nutrient input watershed, high cyanoHABs risk categories have a significantly higher proportion of stormflow than low cyanoHABs risk categories. Within the four nutrient-cyanoHABs risk categories, the cyanobacteria predictions had similar mean precipitations and bimodal distributions. For the microcystin predictions, the precipitation distributions shared a similar pattern, notable however, the low nutrient high microcystin risk category was right skewed, with a significantly lower median.  
...
Table 3. The number and percent of lake predictions within each category for annual anthrpogenic nutrient inputs and cyanoHABs risk.
...
\pagebreak
This is a pagebreak to hopefully stop the above table from somehow connected with the below figures.
