...
```{r moran's I independent errors}

# standardized residuals with spatial dependence modeled
cyano_lake
cyano_nolake

coords <- st_coordinates(cyano_lake$obdata |> st_as_sf(coords = c(".xcoord", ".ycoord")))
nb     <- knn2nb(knearneigh(coords, k = 3))
lw     <- nb2listw(nb, style = "W")

# small p-value -> evidence of autocorrelation
moran.test(rstandard(cyano_lake), lw, alternative = "two.sided")
moran.test(rstandard(cyano_nolake), lw, alternative = "two.sided")

spgmod <- spglm(presence ~ elev + strat, data = moose, family = binomial, spcov_type = "exponential")

summary(spgmod)

# For the spatial logistic models
spgmod <- micx_lake
spgmod <- micx_nolake

errors <- fitted(spgmod, type = "spcov")$ie

eprods <- eigen(covmatrix(spgmod))

errors_sqrtInv <- eprods$vectors %*% diag(1 / sqrt(eprods$values)) %*% t(eprods$vectors)

errors_decor <- as.vector(errors_sqrtInv %*% errors)

coords <- st_coordinates(spgmod$obdata |> st_as_sf(coords = c(".xcoord", ".ycoord")))
 
nb <- knn2nb(knearneigh(coords, k = 3))

lw <- nb2listw(nb, style = "W")

moran.test(errors_decor, lw, alternative = "two.sided")
...
box_cyano <- ggplot(PredData, aes(x = cyano_cat, y = drain_ratio, fill = cyano_cat)) +
  ...
  labs(title = 'Cyanobacteria', x = NULL, y = "Drainage Ratio") +
  theme_bw() +
  guides(fill = FALSE)
...
box_micx <- ggplot(PredData, aes(x = micx_cat, y = drain_ratio, fill = micx_cat)) +
  ...
  coord_cartesian(ylim = c(0, 900)) +
  labs(title = 'Microcystin', x = NULL, y = "Drainage Ratio") +
  theme_bw() +
  guides(fill = FALSE)
