...
ggplot() + 
  ...
  geom_sf(data = std_resid_comb, 
          aes(fill = .std.resid), pch = 21, size = 2) +
  # scale_fill_gradientn(colors = RColorBrewer::brewer.pal(8, 'BrBG'),
  #                      name = "Standardized\nResidual") +
  facet_wrap(~ resp, ncol = 1, nrow = 2) +
  colorspace::scale_fill_continuous_divergingx(palette = "BrBG", mid = 0, name = "Standardized\nResidual") |>
  theme_void()
...
fit_micx <- ggplot() + 
  ...
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, 'PiYG'),
                       name = "Microcystin\nFitted \nProbability") +
  theme_void()
