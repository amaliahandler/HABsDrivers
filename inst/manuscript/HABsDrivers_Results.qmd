...
# 
# drain_clean <- subset(PredData, drain_ratio >= (Q1 - 1.5 * IQR_val) & drain_ratio <= (Q3 + 1.5 * IQR_val))
...
box_cyano <- ggplot(PredData, aes(x = cyano_cat, y = drain_ratio, fill = cyano_cat)) +
 ...
 stat_summary(
    fun = mean,
    geom = "text",
    aes(label = paste(round(after_stat(y), 1))),
    vjust = -0.005,           # Moves the text above the boxplot
    size = 3.5                # Text size
  ) +
  facet_wrap(~AG_ECO3) +
...
ggarrange(plotlist = list(box_cyano, box_micx), ncol = 2, nrow = 1, labels = "auto")
