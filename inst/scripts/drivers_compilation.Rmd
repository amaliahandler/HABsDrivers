...
# Remove observations when there is a lack of microcystin and cyanobacteria data
nla_long <- nla_all_coms %>%
  mutate(
    ...
    MICX_DET = case_when(is.na(MICX) & grepl("ND", MICX_NARS_FLAG) ~ 0,
                         is.na(MICX) & !grepl("ND", MICX_NARS_FLAG) ~ NA,
                         is.finite(MICX) ~ 1)
    ) |>
  filter(!(is.na(MICX_DET) & is.na(B_G_DENS))) |>
  select(SITE_ID, VISIT_NO, UNIQUE_ID, DSGN_CYCLE, DATE_COL, LAT_DD83, LON_DD83, COMID,
         ...
         TURB, NITRATE_N, PH, CHLA_RESULT, MICX, MICX_DET, B_G_DENS) %>%
...
ws_char <- wsmet |>
  select(UNIQUE_ID, DSGN_CYCLE, lakemorpho_fetch, BFIWs, AgKffactWs, KffactWs, RunoffWs, Precip_Minus_EVTWs, Precip8110Ws, Tmean8110Ws, ElevWs, SlopeWs) %>%
...
habs_comb <- nla_long %>%
  ...
  # Remove likely erroneous data
  # There is a MAXDEPTH measurement that is close to 1000 m that is not believable (it's not Crater Lake)
  ...
  # Aggregate several NNI variables
  ...
  # Remove some NNI observations that are extreme outliers (likely idiosyncrasies from the down-scaling process)
  mutate(n_farm_inputs = ifelse(n_farm_inputs > 600, NA, n_farm_inputs),
  ...
  # Round numbers to a reasonable number of significant figures
  mutate(
    DO_SURF = round(DO_SURF, 2),
    CHLA_RESULT = round(CHLA_RESULT, 3),
    MICX = round(MICX, 3),
    B_G_DENS = round(B_G_DENS, 0),
    EVAP_INFL = round(EVAP_INFL, 3),
    across(contains("_ws"), ~ round(., 2)),
    temp_mean_month = round(temp_mean_month , 2),
    lakemorpho_fetch = round(lakemorpho_fetch, 0),
    across(contains("Ws"), ~ round(., 2)),
    across(N_CBNF:N_livestock_Waste, ~ round(., 2)),
    across(contains("P_"), ~ round(., 2)),
    across(n_farm_inputs:n_dev_inputs, ~ round(., 2))
  ) |>
...
habs <- sf::st_transform(habs_nars_all, crs = 5072)
