...
out <- optim(0, 
             fn = find_c, 
             # formula = y ~ x,
             formula = B_G_DENS ~ NTL, AMMONIA_N, DOC, PH, TURB, Precip8110Ws, Tmean8110Ws, EVAP_INFL, MAXDEPTH, lakemorpho_fetch, AG_ECO3 + MAXDEPTH * NTL, 
             data = dat, 
             method = "Brent", 
             lower = -10, upper = 10)
c_value <- exp(out$par)
...
# Save the model
# saveRDS(c4, "./inst/model_objects/model_cyano_withlakedata.rds")
...
cyano_aug <- augment(model_cyano_nolakedata)
cyano_aug$.spde <- fitted(model_cyano_nolakedata, type = "spcov")$de
