...
run_cyano <- function(mod_vars, interact){
  splm(create_formula(dep_var, mod_vars, log = T, const = const, interact),
              ...
              data = habs_mod, # filter(drop_na(habs, all_of(dep_var), all_of(mod_vars)))
...
habs_mod <- habs |>
  select(B_G_DENS, NTL, PTL, NITRATE_N, NITRATE_DET, AMMONIA_N, DOC, PH, TURB, Precip8110Ws, Tmean8110Ws, precip_mean_month, temp_mean_month, EVAP_INFL, MAXDEPTH, p_dev_inputs, n_dev_inputs, lakemorpho_fetch, AG_ECO3, UNIQUE_ID, DSGN_CYCLE) |>
  drop_na()

# Full, best model?
cy1 <- run_cyano(mod_vars = c("NTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH","p_dev_inputs"), interact = NULL)

# Compare EVAP_INFL versus ecoregion. 
cy2 <- run_cyano(mod_vars = c("NTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "Precip8110Ws", "Tmean8110Ws", "AG_ECO3", "MAXDEPTH","p_dev_inputs"), interact = NULL)

# Does TN or TP fit better?
cy3 <- run_cyano(mod_vars = c("PTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH","n_dev_inputs"), interact = NULL)

# Monthly mean temperature and precipitation or climate normals?
cy4 <- run_cyano(mod_vars = c("NTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "precip_mean_month", "temp_mean_month", "EVAP_INFL", "MAXDEPTH","p_dev_inputs"), interact = NULL)

# Does including fetch improve the model?
cy5 <- run_cyano(mod_vars = c("NTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH","p_dev_inputs", "lakemorpho_fetch"), interact = NULL)

# Do interactions improve the model?
cy6 <- run_cyano(mod_vars = c("NTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "p_dev_inputs", "AG_ECO3"), interact = "AG_ECO3 * NTL")

cy7 <- run_cyano(mod_vars = c("NTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "p_dev_inputs"), interact = "MAXDEPTH * NTL")

cy8 <- run_cyano(mod_vars = c("NTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "p_dev_inputs", "AG_ECO3"), interact = "MAXDEPTH * NTL + AG_ECO3 * NTL")

# Compile AIC and loocv data for each model
cyano_nla_comp <- do.call(rbind, lapply(list(cy1, cy2, cy3, cy4, cy5, cy6, cy7), function(mod){
  cv_res <- loocv(mod)
  cv_res$AIC <- AIC(mod)
  coef_names <- names(mod$coefficients$fixed)
  coef_names <- coef_names[!coef_names %in% c("(Intercept)", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB")]
  coefs <- paste(coef_names, collapse = ", ")
  cv_res$coefs_fixed <- coefs
  return(cv_res)
}))

arrange(cyano_nla_comp, AIC)

# Make a table of the model comparison

# Variables common to all models
com_vars <- c("NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "TURB", "MAXDEPTH", "EVAP_INFL")
