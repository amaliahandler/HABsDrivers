...
# Run best fit model (c3) with all available observations with local = F
mod_vars <- c("N_Total_Inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3")
interact <- NULL
dat_final <- habs |>
  select(all_of(mod_vars), B_G_DENS, DSGN_CYCLE, UNIQUE_ID) |>
  drop_na()
...
# Save the model
saveRDS(cyano_nolake_sp, "./inst/model_objects/model_cyano_nolakedata_20260505.rds")
