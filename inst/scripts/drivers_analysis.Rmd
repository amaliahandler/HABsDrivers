...
mx4_sp <-run_micx_sp(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"))

mx5_sp <-run_micx_sp(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"))

mx6_sp <-run_micx_sp(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3", int = "NTL * MAXDEPTH"))

mx_lake_sp_mods <- list(mx1_sp, mx2_sp, mx3_sp, mx4_sp, mx5_sp, mx6_sp)
...
mx_lake_sp_loocv[[1]] <- loocv(mx1_sp, cv_predict = TRUE)
mx_lake_sp_loocv[[2]] <- loocv(mx2_sp, cv_predict = TRUE)
mx_lake_sp_loocv[[3]] <- loocv(mx3_sp, cv_predict = TRUE)
mx_lake_sp_loocv[[4]] <- loocv(mx4_sp, cv_predict = TRUE)
mx_lake_sp_loocv[[5]] <- loocv(mx5_sp, cv_predict = TRUE)
mx_lake_sp_loocv[[6]] <- loocv(mx6_sp, cv_predict = TRUE)
...
saveRDS(mx_lake_sp_modcomp, "./inst/model_objects/micx_spmod_compare_withlakedata.rds")
...
# Save the fitted values (for later plotting in results)
saveRDS(micx_sp_withlake_loocv, "./inst/model_objects/model_micx_withlakedata_loocv.rds")
...
saveRDS(micx_nsp_withlake, "./inst/model_objects/model_micx_withlakedata_nonspat.rds")

# Visualize the results
# Boxplot of probabilities
micx_sp_withlake_aug <- augment(micx_nsp_withlake, type.predict = "response")
levels(micx_sp_withlake_aug$MICX_DET) <- c("Below Detection", "Above Detection")

ggplot(data = micx_sp_withlake_aug, aes(x = as.factor(MICX_DET), y = .fitted)) + 
...
# Comparing models requires the same set of observations be used for all models. Trim the dataset to include just the variables that will be investigated as potential fixed effects in the model.
dat_micx <- habs |>
  select(# Dependent variable and random effects
         MICX_DET, UNIQUE_ID, DSGN_CYCLE, 
         # Climate fixed effects
         ...
         p_dev_inputs, n_dev_inputs, N_Total_Deposition, N_Total_Inputs, p_total_inputs, p_farm_inputs, n_farm_inputs, 
...
# Save the predictions for plotting in the results
saveRDS(micx_sp_nolake_loocv, "./inst/model_objects/model_micx_nolakedata_loocv.rds")
...
saveRDS(micx_nsp_nolake, "./inst/model_objects/model_micx_nolakedata_nsp.rds")

# Visualize the results
# Boxplot of probabilities
micx_sp_nolake_aug <- augment(micx_nsp_nolake, type.predict = "response")
...
ggplot() + 
  ...
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, 'YlOrRd'),
                       name = "Fitted \nProbability") +
  theme_void()
ggsave("./inst/figures/micx_sp_nolake_fitted_probs_map.png", width = 5, height = 3.5, units = "in", dpi = 600)
...
ggplot() + 
  ...
  scale_fill_stepsn(colors = cols,
                    ...
                    name = "Standardized \nResiduals") +
  theme_void()
ggsave("./inst/figures/micx_nolake_stdresiduals_map.png", width = 7, height = 3.5, units = "in", dpi = 600)

# The spatial covariance contribution to fitted values, mapped
micx_aug <- micx_sp_nolake |>
  augment() |>
  mutate(.spde = fitted(micx_sp_nolake, type = "spcov")$de, .after = .std.resid)
