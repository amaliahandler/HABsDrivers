...
# HABs data
dat <- st_drop_geometry(drop_na(select(habs, B_G_DENS, NTL, NITRATE_N, DOC, PH, Precip8110Ws, Tmean8110Ws, EVAP_INFL, MAXDEPTH, lakemorpho_fetch, AG_ECO3)))

# Finds the c for log(y + c) (note that y should be on the original scale for the purposes of this function. The find_c function takes the log of the response variable)
# Note that changing the covariates impacts the constant. By trying different sets of covariates, the constant bounces around a bit, but is usually in the 100's to the 1000's. 
out <- optim(0, 
             fn = find_c, 
             formula = B_G_DENS ~ NTL + NITRATE_N + DOC + PH + EVAP_INFL + Precip8110Ws + Tmean8110Ws + MAXDEPTH + lakemorpho_fetch + AG_ECO3, 
             data = dat, 
             method = "Brent", 
             lower = -10, upper = 10)
c_value <- exp(out$par)
...
habs_mod <- habs |>
  select(B_G_DENS, NTL, PTL, np_ratio, NITRATE_N, DOC, PH, EVAP_INFL,
         Precip8110Ws, Tmean8110Ws, precip_mean_month, temp_mean_month, 
         MAXDEPTH, lakemorpho_fetch, AG_ECO3, UNIQUE_ID, DSGN_CYCLE) |>
  drop_na()
