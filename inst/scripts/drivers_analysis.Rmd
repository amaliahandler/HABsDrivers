...
```{r data prep}
# Create aggregate nutrient input variables
habs <- habs |>
...
# There is a MAXDEPTH measurement that is close to 1000 m that is not believable
...
cy3 <- run_cyano_ml(mod_vars = c("NTL", "NITRATE_N", "AMMONIA_N", "DOC", "PH", "TURB", "precip_mean_month", "temp_mean_month", "EVAP_INFL", "MAXDEPTH"), interact = NULL)
...
# Review model AUC
mx_ie_auc <- sapply(mx_ie_mods, AUROC)
...
micx_sp_withlake_auc <- AUROC(micx_sp_withlake)

# Get bias, MSPE
micx_sp_withlake_loocv <- loocv(micx_sp_withlake)
...
# Get AUC of non-spatial model
micx_nsp_withlake_auc <- AUROC(micx_nsp_withlake)

# Get bias, MSPE of non-spatial model
micx_nsp_withlake_loocv <- loocv(micx_nsp_withlake)
...
# Save the model comparison table
saveRDS(micx_ie_modcomp, "./inst/model_objects/micx_iemod_compare_nolakedata_run3.rds")
...
# Run leave one out cross validation
micx_sp_loocv <- do.call(rbind, lapply(micx_sp_mods, loocv, cv_predict = TRUE))
...
pROC::roc(micx_sp_nolake$obdata$MICX_DET, micx_sp_nolake_loocv$cv_predict)
...
# Get AUC of non-spatial model
micx_nsp_nolake_auc <- AUROC(micx_nsp_nolake)

# Get bias, MSPE of non-spatial model
micx_nsp_nolake_loocv <- loocv(micx_nsp_nolake, cv_predict = TRUE)

# Check AUC for loovc
pROC::roc(micx_nsp_nolake$obdata$MICX_DET, micx_nsp_nolake_loocv$cv_predict)
...
micx_sp_withlake_loocv

micx_nsp_withlake_auc
micx_nsp_withlake_loocv

# No lake data
micx_sp_nolake_auc
micx_sp_nolake_loocv

micx_nsp_nolake_auc
micx_nsp_nolake_loocv

# Comparison table
micx_spatial_comp <-
  rbind(micx_sp_withlake_loocv,
        micx_nsp_withlake_loocv,
        micx_sp_nolake_loocv,
        micx_nsp_nolake_loocv) |>
  mutate(resp_var = "micx",
         lake_data = c(rep("present", 2), rep("absent", 2)),
         spatial_mod = c(rep(c("spatial", "non-spatial"), 2)), .before = bias,
         auc = c(micx_sp_withlake_auc, micx_nsp_withlake_auc, micx_sp_nolake_auc, micx_nsp_nolake_auc))
