...
micx_spatial_comp <-
  rbind(micx_sp_withlake_loocv,
        micx_nsp_withlake_loocv) |>
  mutate(resp_var = "cyano",
         lake_data = c(rep("present", 2)), # lake_data = c(rep("present", 2), rep("absent", 2)),
         spatial_mod = c("spatial", "non-spatial"), .before = bias, #spatial_mod = c(rep(c("spatial", "non-spatial"), 2)), .before = bias,
         auc = c(micx_sp_withlake_auc, micx_nsp_withlake_auc))
