...
cymod_final_dat <- drop_na(habs, all_of(dep_var), all_of(mod_vars))

cymod_final_withlake <- splm(create_formula(dep_var, mod_vars, log = T, const = const, interact),
                             data = cymod_final_dat,
                             spcov_type = "exponential",
                             random = ~ DSGN_CYCLE + UNIQUE_ID,
                             local = F)
  
# Evaluate fit of best model
cyano_withlake_loocv <- loocv(cymod_final_withlake, cv_predict = TRUE)

cymod_final_dat$preds <- cyano_withlake_loocv$cv_predict

max_val <- max(log10(cymod_final_dat$B_G_DENS + const), cymod_final_dat$preds)

fig <- ggplot(data = cymod_final_dat, aes(x = preds, y = log10(B_G_DENS + const))) +
  geom_hex(bins = 60) +
...
# Which models are included in the table?
cc_list <- list(c1, c2, c3, c4, c5, c6)
...
# Run the best fit model, cy5
mod_vars <- c("n_farm_inputs", "p_dev_inputs", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "BFIWs", "MAXDEPTH", "lakemorpho_fetch")

interact <- NULL

cmod_final_dat <- drop_na(habs, all_of(dep_var), all_of(mod_vars))

cmod_final_nolake <- splm(create_formula(dep_var, mod_vars, log = T, const = const, interact),
                          data = cmod_final_dat,
                          spcov_type = "exponential",
                          random = ~ DSGN_CYCLE + UNIQUE_ID,
                          local = F)
...
# Evaluate fit of best model
cyano_nolake_loocv <- loocv(cmod_final_nolake, cv_predict = TRUE)

cmod_final_dat$preds <- cyano_nolake_loocv$cv_predict

max_val <- max(log10(cmod_final_dat$B_G_DENS + const), cmod_final_dat$preds)

fig <- ggplot(data = cmod_final_dat, aes(x = preds, y = log10(B_G_DENS + const))) +
  geom_hex(bins = 60) +
  scale_fill_continuous(type = "viridis") +
  xlim(2.8, max_val) + ylim(2.8, max_val) +
  ylab("Observed") +
...
# Save the model
# saveRDS(c2, "./inst/model_objects/model_cyano_withlakedata.rds")
...
ggplot() + 
  ...
  geom_sf(data = arrange(cmod_final_dat, preds), 
          aes(fill = preds), pch = 21, size = 2) +
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, 'YlOrRd'),
                       name = "Log10(Cyanobacteria + 1000)") +
  theme_void()
ggsave("./inst/figures/cyano_sp_final_cyano_preds_map.png", width = 6, height = 3.5, units = "in", dpi = 600)
...
m1 <- run_micx_ie(mod_vars = c("p_farm_inputs", "n_dev_inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "BFIWs", "AG_ECO3"), int = NULL)
...
# Review model AIC/BIC
glances(m0, m1, m2, m3, m4, m5)

# Make a list of the independent error models to compare
micx_ie_mods <- list(m0, m1, m2, m3, m4, m5)
...
# Put it all together into a table, note that the table is organized by BIC
micx_ie_modcomp <- glances(m0, m1, m2, m3, m4, m5) |>
  arrange(model) |>
  cbind(micx_ie_loocv) |>
  select(-npar, -value, -AICc, -logLik, -pseudo.r.squared) |> 
...
# Make a list of the models to compare
micx_sp_mods <- list(m3_sp, m4_sp, m5_sp)
...
micx_sp_modcomp <- micx_sp_loocv |>
  mutate(model = c("m3_sp", "m4_sp", "m5_sp"),
         AUC = micx_sp_auc,
         distinct_vars = dif_vars,
         common_vars = paste(com_vars, collapse = ", ")) |>
  arrange(desc(AUC)) |>
...
# Map of the PNW
ggplot() + 
  geom_sf(data = pnw, fill = "#e2e2e2", color = "white", lwd = 1) +
  geom_sf(data = pnw_micx, aes(fill = .fitted), pch = 21, size = 3) +
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, 'YlOrRd'),
                       name = "Estimated \nProbability") +
  theme_void() +
...
# Map of all NHD lakes >1 ha in area in the PNW
ggplot() + 
  geom_sf(data = pnw, fill = "#e2e2e2", color = "white", lwd = 1) +
  geom_sf(data = pnw_lk_pts, pch = 21, fill = "blue", alpha = 0.5, size = 1) +
  theme_void()
