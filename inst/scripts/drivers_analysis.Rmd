...
# Calculate N/P ratio
# Calculate total N and P inputs
habs <- habs |>
  mutate(
    AG_ECO3 = factor(AG_ECO3, levels = c("WMTNS", "PLNLOW", "EHIGH")),
    np_ratio = NTL/PTL,
...
habs_mod <- habs |>
  select(B_G_DENS, NTL, PTL, np_ratio, NITRATE_N, AMMONIA_N, DOC, PH, TURB, Precip8110Ws, Tmean8110Ws, precip_mean_month, temp_mean_month, EVAP_INFL, MAXDEPTH, lakemorpho_fetch, AG_ECO3, UNIQUE_ID, DSGN_CYCLE) |>
  drop_na()
...
# Does TN, TP, or TN/TP fit better?
cy1 <- run_cyano_ml(mod_vars = c("NTL", "DOC", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"), interact = NULL)

cy2 <- run_cyano_ml(mod_vars = c("PTL", "DOC", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"), interact = NULL)

cy3 <- run_cyano_ml(mod_vars = c("np_ratio", "DOC", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"), interact = NULL)

# Monthly mean temperature and precipitation or climate normals?
cy4 <- run_cyano_ml(mod_vars = c("NTL", "DOC", "precip_mean_month", "temp_mean_month", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"), interact = NULL)

# Are there ecoregional differences?
cy5 <- run_cyano_ml(mod_vars = c("NTL", "DOC", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

# Does additional in-lake water data improve the model?
cy6 <- run_cyano_ml(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)
...
# 
# # How does simplifying the model to better match microcystin impact the model?
# cy7 <- run_cyano_ml(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)
# 
# # What about the N/P ratio?
# cy8 <- run_cyano_ml(mod_vars = c("np_ratio", "NITRATE_N", "AMMONIA_N", "DOC", "PH", "TURB", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = "AG_ECO3 * NTL")

# Make a table of the model comparison

# Variables common to all models
com_vars <- c("DOC", "MAXDEPTH", "EVAP_INFL", "lakemorpho_fetch")
...
saveRDS(cy_res_table, "./inst/model_objects/cyano_mod_compare_withlakedata_20260429.rds")

# Run best fit model (c6) with all available oberservations with local = F
mod_vars <- c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3") 
interact <- NULL
dat_final <- habs |>
...
# Save the model
# saveRDS(cyano_withlake_sp, "./inst/model_objects/model_cyano_withlakedata_20260429.rds")
...
habs_mod <- habs |>
  select(B_G_DENS, Precip8110Ws, Tmean8110Ws, precip_mean_month, temp_mean_month, BFIWs, MAXDEPTH, lakemorpho_fetch, p_dev_inputs, p_farm_inputs, n_farm_inputs, n_dev_inputs, N_Total_Inputs, p_total_inputs, fst_ws, wet_ws,KffactWs, AG_ECO3, UNIQUE_ID, DSGN_CYCLE) |>
  drop_na()
...
c2 <- run_cyano_ml(mod_vars = c("p_farm_inputs", "n_dev_inputs", "Precip8110Ws", "Tmean8110Ws", "BFIWs", "MAXDEPTH", "lakemorpho_fetch"), interact = NULL)

# Climate normals versus monthly precip and temp
c3 <- run_cyano_ml(mod_vars = c("n_farm_inputs", "p_dev_inputs", "precip_mean_month", "temp_mean_month", "BFIWs", "MAXDEPTH", "lakemorpho_fetch"), interact = NULL)

# Ecoregions
c4 <- run_cyano_ml(mod_vars = c("n_farm_inputs", "p_dev_inputs", "Precip8110Ws", "Tmean8110Ws", "BFIWs", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

# Other land cover
c5 <- run_cyano_ml(mod_vars = c("n_farm_inputs", "p_dev_inputs", "fst_ws", "wet_ws", "Precip8110Ws", "Tmean8110Ws", "BFIWs", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)
...
# Base model, in-lake model with a few variables for nutrient inputs
c1 <- run_cyano_ml(mod_vars = c("n_farm_inputs", "p_dev_inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

# Source of N and P switched
c2 <- run_cyano_ml(mod_vars = c("p_farm_inputs", "n_dev_inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

# What about using the total nutrient inputs?
c3 <- run_cyano_ml(mod_vars = c("N_Total_Inputs", "p_total_inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

# Other land cover
c4 <- run_cyano_ml(mod_vars = c("n_farm_inputs", "p_dev_inputs", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

c5 <- run_cyano_ml(mod_vars = c("n_farm_inputs", "p_dev_inputs", "BFIWs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

c6 <- run_cyano_ml(mod_vars = c("n_farm_inputs", "p_dev_inputs", "KffactWs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

# Variables common to all models
com_vars <- c("Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3PLNLOW", "AG_ECO3EHIGH")

# Which models are included in the table?
cy_list <- list(c1, c2, c3, c4, c5, c6)

# Save the models
# saveRDS(cy_list, "./inst/model_objects/cyano_mods_nolakedata_20260506.rds")

# Get the fixed effects that are different between the models
dif_vars <- sapply(cy_list, function(mod){
  # ecor <- ifelse("AG_ECO3" %in% com_vars),
  # com_vars <- ifelse(ecor, com_vars[!com_vars == "AG_ECO3"]),
  # com_vars <- c(com_vars, "AG_ECO3PLNLOW", "PLNLOW"),
  coef_names <- names(mod$coefficients$fixed)
  coef_names <- coef_names[!coef_names %in% c("(Intercept)", com_vars)]
  paste(coef_names, collapse = ", ")
})
...
# Save the model comparison table for inclusion in supplement
saveRDS(cmods_ml_nolake, "./inst/model_objects/cyano_mod_compare_nolakedata_20260506.rds")

# Run best fit model (c1) with all available observations with local = F
mod_vars <- c("n_farm_inputs", "p_dev_inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3")
interact <- NULL
dat_final <- habs |>
  select(all_of(mod_vars), B_G_DENS, DSGN_CYCLE, UNIQUE_ID) |>
  drop_na()
...
# Save the model
saveRDS(cyano_nolake_sp, "./inst/model_objects/model_cyano_nolakedata_20260507.rds")
  
# Run the best fit model, c3, without spatial correlation for comparison
cmod_final_nolake_nsp <- splm(create_formula(dep_var, mod_vars, log = T, const = const, interact),
                              data = dat_final,
                              spcov_type = "none",
                              local = F)
  
# Evaluate fit of spatial and non-spatial models
...
# Save the table
saveRDS(cyano_spatial_comp, "./inst/model_objects/spatial_compare_cyano_20260512.rds")
...
dat_micx_lake <- habs |>
  select(# Dependent variable and random effects
         ...
         NTL, NITRATE_N, DOC, TEMPERATURE, EVAP_INFL, PH) |>
  drop_na()
...
mx3 <-run_micx_ie(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"))

# What about additional watershed metrics?
mx4 <-run_micx_ie(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "p_farm_inputs", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"))

# What about ecoregion?
mx5 <-run_micx_ie(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"))

# What is there an interaction?
mx6 <-run_micx_ie(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "p_farm_inputs", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3", int = "NTL * MAXDEPTH"))

# # What if we try better matching the cyano in-lake model?
# mx7 <-run_micx_ie(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"))
...
# The code below has failed to run 4 times
# mx_ie_loovc <- lapply(mx_ie_mods, loocv, cv_predict = TRUE)
mix_ie_loocv <- vector(mode = "list", length = length(mx_ie_mods))

mix_ie_loocv[[1]] <- loocv(mx1, cv_predict = TRUE)
mix_ie_loocv[[2]] <- loocv(mx2, cv_predict = TRUE)
mix_ie_loocv[[3]] <- loocv(mx3, cv_predict = TRUE)
mix_ie_loocv[[4]] <- loocv(mx4, cv_predict = TRUE)
mix_ie_loocv[[5]] <- loocv(mx4, cv_predict = TRUE)
mix_ie_loocv[[6]] <- loocv(mx6, cv_predict = TRUE)
# mix_ie_loocv[[7]] <- loocv(mx7, cv_predict = TRUE)

mx_ie_loovc <- mix_ie_loocv[1:6]
...
saveRDS(mx_ie_modcomp, "./inst/model_objects/micx_iemod_compare_withlakedata_20260420.rds")
...
mx1_sp <- run_micx_sp(mod_vars = c("NTL", "DOC", "precip_mean_month", "TEMPERATURE", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"))

mx2_sp <-run_micx_sp(mod_vars = c("NTL", "DOC", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"))

mx3_sp <-run_micx_sp(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"))

mx4_sp <-run_micx_sp(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "p_farm_inputs", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch"))

mx5_sp <-run_micx_sp(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"))

mx6_sp <-run_micx_sp(mod_vars = c("NTL", "NITRATE_N", "DOC", "PH", "p_farm_inputs", "Precip8110Ws", "Tmean8110Ws", "EVAP_INFL", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3", int = "NTL * MAXDEPTH"))

# Make a list of the models to compare
mx_lake_sp_mods <- list(mx4_sp, mx5_sp, mx6_sp)

mx_lake_sp_mods2 <- list(mx1_sp, mx2_sp, mx3_sp, mx4_sp, mx5_sp, mx6_sp)

# Run leave one out cross validation
mx_lake_sp_loocv <- lapply(mx_lake_sp_mods, loocv, cv_predict = TRUE)

# Run loocv sequentially since lapply seems to hang on my personal computer
mx_lake_sp_loocv <- vector(mode = "list", length = length(mx_lake_sp_mods))

mx_lake_sp_loocv[[1]] <- loocv(mx4_sp, cv_predict = TRUE)
mx_lake_sp_loocv[[2]] <- loocv(mx5_sp, cv_predict = TRUE)
mx_lake_sp_loocv[[3]] <- loocv(mx6_sp, cv_predict = TRUE)

# Once mods 1, 2, and 3 are done running, run the LOOCV below
mx_lake_sp_loocv2 <- vector(mode = "list", length = length(mx_lake_sp_mods2))

# Add the previously run models to the list
mx_lake_sp_loocv2[[4]] <- mx_lake_sp_loocv[[1]]
mx_lake_sp_loocv2[[5]] <- mx_lake_sp_loocv[[2]]
mx_lake_sp_loocv2[[6]] <- mx_lake_sp_loocv[[3]]

# Run the new spatial models
mx_lake_sp_loocv2[[1]] <- loocv(mx4_sp, cv_predict = TRUE)
mx_lake_sp_loocv2[[2]] <- loocv(mx5_sp, cv_predict = TRUE)
mx_lake_sp_loocv2[[3]] <- loocv(mx6_sp, cv_predict = TRUE)

# Save the revised spatial models
saveRDS(mx_lake_sp_mods2, "./inst/model_objects/mx_lake_sp_6modobjs.rds")
saveRDS(mx_lake_sp_loocv2, "./inst/model_objects/mx_lake_sp_loocv_6mods.rds")

# 
mx_lake_sp_mods <- mx_lake_sp_mods2
mx_lake_sp_loocv <- mx_lake_sp_loocv2

# Check that everything run here. If yes, overwrite the mx_lake_sp_loocv object with the complete 6 models.
...
mx_lake_sp_modcomp <- mx_lake_sp_fit |>
  mutate(model = c("mx1_sp", "mx2_sp", "mx3_sp", "mx4_sp", "mx5_sp", "mx6_sp"),
         AUC = mx_lake_sp_auc,
         distinct_vars = dif_vars,
         common_vars = paste(com_vars, collapse = ", ")) |>
  arrange(desc(AUC))
...
# Save the model comparison table
saveRDS(mx_lake_sp_modcomp, "./inst/model_objects/mixc_spmod_compare_withlakedata_20260423.rds")
...
mod_vars <- c("NTL", "NITRATE_N", "DOC", "PH", "EVAP_INFL", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3")
int <- NULL
dat_final <- habs |>
...
saveRDS(micx_sp_withlake, "./inst/model_objects/model_micx_withlakedata_20260513.rds")
saveRDS(micx_sp_withlake_auc, "./inst/model_objects/model_micx_withlakedata_auc_20260513.rds")
saveRDS(micx_sp_withlake_loocv, "./inst/model_objects/model_micx_withlakedata_loovc_20260513.rds")
...
saveRDS(micx_nsp_withlake, "./inst/model_objects/model_micx_withlakedata_nonspat_20260513.rds")
saveRDS(micx_nsp_withlake_auc, "./inst/model_objects/model_micx_withlakedata_nonspat_auc_20260513.rds")
saveRDS(micx_nsp_withlake_loocv, "./inst/model_objects/model_micx_withlakedata_nonspat_loocv_20260513.rds")
...
dat_micx <- habs |>
  select(# Dependent variable and random effects
         ...
         p_dev_inputs, n_dev_inputs, N_Total_Deposition, N_Total_Inputs, p_total_inputs, p_farm_inputs, n_farm_inputs, fst_ws, wet_ws, KffactWs) |> 
...
# Version 2 models for Ecol Apps revisions

# Base model, in-lake model with a few variables for nutrient inputs
m1_sp <- run_micx_sp(mod_vars = c("n_farm_inputs", "p_dev_inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), int = NULL)

# Source of N and P switched
m2_sp <- run_micx_sp(mod_vars = c("p_farm_inputs", "n_dev_inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), int = NULL)

# What about using the total nutrient inputs?
m3_sp <- run_micx_sp(mod_vars = c("N_Total_Inputs", "p_total_inputs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), int = NULL)

# Other land cover
m4_sp <- run_micx_sp(mod_vars = c("n_farm_inputs", "p_dev_inputs", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), int = NULL)

m5_sp <- run_micx_sp(mod_vars = c("n_farm_inputs", "p_dev_inputs", "BFIWs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), int = NULL)

m6_sp <- run_micx_sp(mod_vars = c("n_farm_inputs", "p_dev_inputs", "KffactWs", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "AG_ECO3"), int = NULL)

# Make a list of the models to compare
micx_sp_mods <- list(m1_sp, m2_sp, m3_sp, m4_sp, m5_sp, m6_sp)

# Save the models
# saveRDS(micx_sp_mods, "./inst/model_objects/micx_mods_nolakedata_20260506.rds")

# Run leave one out cross validation
# micx_sp_loocv <- lapply(micx_sp_mods, loocv, cv_predict = TRUE)

# Run the loocv in parts
m1_sp_loocv <- loocv(m1_sp, cv_predict = TRUE)
m2_sp_loocv <- loocv(m2_sp, cv_predict = TRUE)
m3_sp_loocv <- loocv(m3_sp, cv_predict = TRUE)
m4_sp_loocv <- loocv(m4_sp, cv_predict = TRUE)
m5_sp_loocv <- loocv(m5_sp, cv_predict = TRUE)
m6_sp_loocv <- loocv(m6_sp, cv_predict = TRUE)

# Compile in the list
micx_sp_loocv <- list(m1_sp_loocv, m2_sp_loocv, m3_sp_loocv,
                      m4_sp_loocv, m5_sp_loocv, m6_sp_loocv)
...
micx_sp_loocv_stats <- do.call(rbind, lapply(1:length(micx_sp_loocv), function(i){
  micx_sp_loocv[[i]]$stats
}))

# Put it all together into a table
micx_sp_modcomp <- micx_sp_loocv_stats |>
  mutate(model = c("m1_sp", "m2_sp", "m3_sp", "m4_sp", "m5_sp", "m6_sp"),
...
# Save the model comparison table
saveRDS(micx_sp_modcomp, "./inst/model_objects/micx_spmod_compare_nolakedata_20260507.rds")
...
micx_sp_nolake <- readRDS(here("inst", "model_objects", "model_micx_nolakedata_20260507.rds"))
...
saveRDS(micx_sp_withlake_auc, "./inst/model_objects/model_micx_nolakedata_auc_20260513.rds")
saveRDS(micx_sp_withlake_loocv, "./inst/model_objects/model_micx_nolakedata_loovc_20260513.rds")

Sys.time()

# Save the best performing micx model so far
saveRDS(micx_sp_nolake, "./inst/model_objects/model_micx_nolakedata_20260507.rds")
...
saveRDS(micx_nsp_nolake, "./inst/model_objects/model_micx_nolakedata_nsp_20260514.rds")
saveRDS(micx_sp_withlake_auc, "./inst/model_objects/model_micx_nolakedata_auc_nsp_20260514.rds")
saveRDS(micx_sp_withlake_loocv, "./inst/model_objects/model_micx_nolakedata_loovc_nsp_20260514.rds")
...
# Save the table
saveRDS(micx_spatial_comp, "./inst/model_objects/spatial_compare_micx_20260514.rds")
