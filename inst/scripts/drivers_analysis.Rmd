...
run_micx <- function(mod_vars, int = NULL){
  spglm(create_formula(dep_var = "MICX_DET", ind_vars = mod_vars, interact = int),
                ...
                estmethod = "ml",
                random = ~ DSGN_CYCLE + UNIQUE_ID,
                # Fix the range for the spatial covariance to keep from getting out of control
                # spcov_initial = spcov_initial("exponential", range = 7.5e4, known = "range"),
...
# Filter to the same set of observations for model comparison
habs_mod <- habs |>
  select(MICX_DET, Precip8110Ws, Tmean8110Ws, Precip_Minus_EVTWs, precip_mean_month, temp_mean_month, BFIWs, MAXDEPTH, lakemorpho_fetch, p_dev_inputs, N_Total_Deposition, N_Surplus, p_farm_inputs, n_farm_inputs, fst_ws, wet_ws, KffactWs, RunoffWs, AG_ECO3, UNIQUE_ID, DSGN_CYCLE) |>
  drop_na()
...
# Which climate data?
m4 <- run_micx(mod_vars = c("n_farm_inputs", "fst_ws", "precip_mean_month", "temp_mean_month", "MAXDEPTH", "lakemorpho_fetch", "BFIWs", "AG_ECO3"), int = NULL)

# Runoff or baseflow index?
m5 <- run_micx(mod_vars = c("n_farm_inputs", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "RunoffWs", "AG_ECO3"), int = NULL)

# Will another inventory variable fit in the model?
m6 <- run_micx(mod_vars = c("n_farm_inputs", "p_dev_inputs", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "BFIWs", "AG_ECO3"), int = NULL)
...
# Interactions
m9 <- run_micx(mod_vars = c("p_farm_inputs", "N_Surplus", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "BFIWs", "AG_ECO3"), int = "n_farm_inputs * MAXDEPTH")

m10 <- run_micx(mod_vars = c("p_farm_inputs", "N_Surplus", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "BFIWs", "AG_ECO3"), int = "n_farm_inputs * AG_ECO3")

# Run with the range_constrain = TRUE
m9a <- run_micx(mod_vars = c("p_farm_inputs", "N_Surplus", "fst_ws", "Precip8110Ws", "Tmean8110Ws", "MAXDEPTH", "lakemorpho_fetch", "BFIWs", "AG_ECO3"), int = "n_farm_inputs * MAXDEPTH")

m4a <- run_micx(mod_vars = c("n_farm_inputs", "fst_ws", "precip_mean_month", "temp_mean_month", "MAXDEPTH", "lakemorpho_fetch", "BFIWs", "AG_ECO3"), int = NULL)
...
# Which models are included in the table?
m_list <- list(m2, m3, m4, m5, m6, m7, m8, m9, m10)
...
# Put it all together into a table
cy_res_table <- glances(m2, m3, m4, m5, m6, m7, m8, m9, m10) |>
  arrange(model) |>
  # cbind(micx_loocv) |>
  select(model, n, p, npar, AIC, AICc, BIC) |> # , bias, MSPE, RMSPE
  mutate(AUC = mods_auc) |>
  mutate(distinct_vars = dif_vars,
         common_vars = paste(com_vars, collapse = ", ")) |>
  arrange(BIC) |>
