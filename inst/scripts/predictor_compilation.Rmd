...
eco3 <- read.csv("C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/source_data/NLA_SampleFrame_L3L4Ecoregions 1.csv")
...
PredData <- merge(PredData, ncldMas, by = 'COMID')
...
PredData <- PredData |>
  mutate(N_CBNF = mean(c(n_cbnf, N_CBNF_2007)),
         .keep = 'unused')
...
# combining and compiling lakemorpho depths with exisiting depths
...
# code for creating the lake depths, not necessary to run unless producing new data
...
fetch <- read.csv("C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/source_data/lake_fetch_lm.csv")
