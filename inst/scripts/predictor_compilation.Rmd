...
get_nlcd <- function(coms){
  StreamCatTools::lc_get_data(metric = 'PctWdWet2016, PctUrbMd2016, PctUrbLo2016,PctUrbHi2016,
...
# divide existing predictor data set into groups of 10,000 by COMID 
# increasing efficiency in pulling NLCD data
com_vector <- PredData$COMID
chunks <- split(PredData$COMID, ceiling(seq_along(PredData$COMID) / 500))
chunky <- squash(chunks)
...
test <- lapply(split(PredData$COMID, 1:nrow(PredData)), get_nlcd)

df <- lc_get_data(metric = 'pctmxfst2016,pctdecid2016,pctconif2016,pcturbop2016, pcthbwet2016,pctwdwet2016,pcturbmd2016,pcturblo2016,pcthay2016,pctcrop2016',
              aoi='ws',
              comid = test,
              showAreaSqKm = TRUE)

# no spaces allowed in streamcat world
