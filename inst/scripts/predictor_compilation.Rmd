...
ncldMas <- ncldMas |>
  rowwise() |>
  mutate(fst_ws = pctmxfst2016ws + pctdecid2016ws + pctconif2016ws,
         COMID = comid,
...
# pull in watershed boundary data set using the key and desired data layer
...
# remove all variables with metrics aggregated at the catchment scale
# mean nutrient data from 2002, 2007, and 2012 to create averaged value for nutrient inputs
# keeping only new variables and ones existing already in predictor data set
nutrMas <- nutrMas |>
...
# join lake metrics to existing predictor data set 
...
PredData <- PredData |>
  rowwise() |> 
  mutate(area_km = lake_area / 1000000,
         ad_ratio = (sqrt(area_km)) / lake_m_depth) 
...
PredData <- PredData |>
  ...
  dplyr::select(c(COMID, n_dev_inputs, n_farm_inputs, p_dev_inputs, p_farm_inputs,
                  BFIWs, MAXDEPTH,lakemorpho_fetch, lake_area, ad_ratio,
                  Precip8110Ws, Tmean8110Ws,
                  fst_ws, AG_ECO3,
                  DSGN_CYCLE, UNIQUE_ID, geometry)) |>
  drop_na()
...
# use spmodel function to run cyano model
# add system time stamps to record processing time
Sys.time()
PredData$preds <- predict(object = model_cyano_nolakedata, newdata = PredData, 
                local = list(method = 'all', parallel = TRUE, ncores = 48),
                interval = 'prediction') # run with 95% prediction intervals
Sys.time()

# perform log transformation on cyano data
PredData <- PredData |>
  mutate(pred_cyano = preds,
         cyano_transform = 10^Pred - 1000)

# run the micx model
Sys.time()
PredData$micxpreds <- predict(object = model_micx_nolakedata, newdata = PredData, 
                local = list(method = 'all', parallel = TRUE, ncores = 48),
                interval = 'prediction') # run with 95% prediction intervals
Sys.time()

# perform transformation on the micx data 
PredData <- PredData |>
  mutate(pred_micx = micxpreds,
         micx_transform = 10^Pred - 1000)

PredData$pred_micx <- boot::inv.logit(PredData$pred_micx)

# saveRDS(pred_df, file = "C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/source_data/predictions/cyano_predictions_2026.rds")
# 
# saveRDS(micx_pred_df, file = "C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/source_data/predictions/micx_predictions_2026.rds")

```

```{r, eval = FALSE}

# for easier data use / analysis, pulling in predictions that were run in the cloud
# load cyano predictions
# load(file = "C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/source_data/predictions/cyano_predictions_2026.rds")
# 
# load micx predictions
# load(file = "C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/source_data/predictions/micx_predictions_2026.rds")

```

```{r, eval = FALSE}

# add predictions to final PredData set
PredData <- pred_df |>
  dplyr::select(Shape, pred_cyano, cyano_transform) |>
  st_join(PredData) |>
  drop_na()

PredData <- micx_pred_df |>
  dplyr::select(Shape, pred_micx, micx_transform) |>
  st_join(PredData) |>
  drop_na() |>
  distinct(COMID, .keep_all = TRUE)
...
PredData <- PredData |>
  mutate(pred_micx_fit = (pred_micx[, "fit"]),
         pred_micx_lwr = (pred_micx[, "lwr"]),
         pred_micx_upr = (pred_micx[, "upr"]),
         pred_cyano_fit = (pred_cyano[, "fit"]),
         pred_cyano_lwr = (pred_cyano[, "lwr"]),
         pred_cyano_upr = (pred_cyano[, "upr"]),
         micx_transform_fit = (micx_transform[, "fit"]),
         micx_transform_lwr = (micx_transform[, "lwr"]),
         micx_transform_upr = (micx_transform[, "upr"]),
         cyano_transform_fit = (cyano_transform[, "fit"]),
         cyano_transform_lwr = (cyano_transform[, "lwr"]),
         cyano_transform_upr = (cyano_transform[, "upr"]),
         .keep = 'unused') |>
  relocate(COMID) |>
  dplyr::select(-c(preds, micxpreds))
...
# Generate models for spatial analysis
```{r, eval = FALSE}
...
# Create models needed for ANOVAs in Results

bf_model <- splm(BFIWs ~ all_pred, comp_cyano, spcov_type = "exponential")
#anova(bf_model)
saveRDS(bf_model, file = 'C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/model_objects/bf_model_cyano.rds')

ad_model <- splm(log10(ad_ratio) ~ all_pred, comp_cyano, spcov_type = "exponential")
#anova(ad_model)
saveRDS(ad_model, file = 'C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/model_objects/ad_model_cyano.rds')

precip <- splm(log10(Precip8110Ws) ~ all_pred, comp_cyano, spcov_type = "exponential")
#anova(precip)
saveRDS(precip, file = 'C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/model_objects/precip_model_cyano.rds')
...
#anova(bfm_model)
saveRDS(bfm_model, file = 'C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/model_objects/bf_model_micx.rds')


adm_model <- splm(log10(ad_ratio) ~ all_pred, comp_micx, spcov_type = "exponential")
#anova(adm_model)
saveRDS(adm_model, file = 'C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/model_objects/ad_model_micx.rds')


precipm <- splm(log10(Precip8110Ws) ~ all_pred, comp_micx, spcov_type = "exponential")
#anova(precipm)
saveRDS(precipm, file = 'C:/Users/mreyno04/OneDrive - Environmental Protection Agency (EPA)/Profile/REPOS/HABsDrivers/inst/model_objects/precip_model_micx.rds')
