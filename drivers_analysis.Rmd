...
```{r model setup}
# Function that creates the formula. When the response variable is cyanobacteria cell counts, add a constant (default is 0) and log-transform.
...
# Robert suggesting using agricultural inputs rather than land cover
habs <- mutate(habs, excess_ag_n_inputs = N_CBNF + N_Fert_Farm + N_livestock_Waste - N_Crop_N_Rem)
...
dep_var  <- "B_G_DENS"
mod_vars <- c("NTL", "PTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "D_EXCESS", "MAXDEPTH", "excess_ag_n_inputs")
cyano_linear    <- splm(create_formula(dep_var, mod_vars, log = T, const = 1000),
                        data = filter(drop_na(habs, all_of(dep_var), all_of(mod_vars))),
                        spcov_type = "exponential",
                        random = ~ DSGN_CYCLE + UNIQUE_ID,
                        local = T)

cyano_nbinom    <- spglm(round(B_G_DENS, 0) ~ NTL + NITRATE_N + NITRATE_DET + AMMONIA_N + DOC + 
                            PH + Precip8110Ws + Tmean8110Ws + D_EXCESS + MAXDEPTH + agr_ws + 
                            AG_ECO3,
                          data = filter(drop_na(habs, all_of(dep_var), all_of(mod_vars))),
                          family = "nbinomial",
                          spcov_type = "exponential",
...
cyano_loocv <- loocv(cyano_linear, cv_predict = TRUE)

cyano_pred$preds <- cyano_loocv$cv_predict

max_val <- max(log10(cyano_pred$B_G_DENS +1000), cyano_pred$preds)
name <- "Log10(Cyanobacteria + 100)"

fig <- ggplot(data = cyano_pred, aes(x = preds, y = log10(B_G_DENS +100))) +
  geom_point(color = "#2A6BA6", alpha=0.3, size = 2) +
  xlim(2, max_val) + ylim(2, max_val) +
  ylab("Observed") +
  xlab("Fitted") # +
  # ggtitle(name)
...
# Finding a constant to add to cyano biovolume
# function to find c
...
micx8 <- run_micx(mod_vars = c("NTL","NITRATE_DET","NITRATE_N", "AMMONIA_N", "PH", "dev_ws", "agr_ws", "KffactWs", "temp_mean_month", "BFIWs", "MAXDEPTH", "EVAP_INFL", "lakemorpho_fetch", "AG_ECO3"))
