...
habs <- mutate(habs, MICX_DET = ifelse(is.finite(MICX), 1, 0))

dep_var  <- "MICX_DET"

mod_vars <- c("NTL", "PTL", "DOC", "PH", "TURB","DO_SURF", "dev_ws", "precip_mean_month", "temp_mean_month","TEMPERATURE", "BFIWs", "EVAP_INFL", "MAXDEPTH", "agr_ws", "AG_ECO3")

micx    <- splm(create_formula(dep_var, mod_vars, log = F),
                family = "binomial",
                data = drop_na(habs, all_of(dep_var), all_of(mod_vars)),
                spcov_type = "exponential",
                random = ~ DSGN_CYCLE + UNIQUE_ID,
...
modRF <- splmRF(create_formula(dep_var, all_ind_vars, log = F),
                data = drop_na(habs, all_of(dep_var), all_of(all_ind_vars)),
                spcov_type = "exponential",
                random = ~ DSGN_CYCLE + UNIQUE_ID,
                # random = ~ DSGN_CYCLE,
