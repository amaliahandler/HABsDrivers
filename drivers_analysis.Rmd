...
cor_dat <- habs |>
  st_drop_geometry() |>
  select(TEMPERATURE, MAXDEPTH, AMMONIA_N, DOC:NITRATE_N, PH, D_EXCESS, fst_ws, wet_ws, lakemorpho_fetch, BFIWs, KffactWs:Precip_Minus_EVTWs, Precip8110Ws, Tmean8110Ws, N_Surplus, N_Total_Deposition, P_Legacy_P, P_Surplus, n_farm_inputs:p_dev_inputs) |>
  relocate(NTL, PTL, NITRATE_N, AMMONIA_N, DOC, PH, TEMPERATURE, TURB, D_EXCESS, fst_ws, wet_ws, KffactWs, n_farm_inputs, p_farm_inputs, n_dev_inputs, p_dev_inputs, N_Surplus, P_Surplus, N_Total_Deposition, P_Legacy_P, Precip_Minus_EVTWs, Precip8110Ws, Tmean8110Ws, RunoffWs, BFIWs, MAXDEPTH, lakemorpho_fetch)
...
cormat_pred <- cor(cor_dat, use = "pairwise.complete.obs")
...
cyano <- splm(create_formula(dep_var, mod_vars, log = T, const = const, 
                             interact = "p_farm_inputs * AG_ECO3"),
              data = filter(drop_na(habs, all_of(dep_var), all_of(mod_vars))),
              spcov_type = "exponential",
              random = ~ DSGN_CYCLE + UNIQUE_ID,
              local = T)
...
# Model comparison for final model without lake water data
...
# No ecoregion or interaction terms
...
cyano2 <- run_cyano(mod_vars = c("BFIWs", "Tmean8110Ws", "Precip8110Ws", "n_farm_inputs", "n_dev_inputs", "p_farm_inputs", "lake_dep", "lakemorpho_fetch", "AG_ECO3"), interact = NULL)

# Add ecoregion interactions with the nutrient inputs
cyano3 <- run_cyano(mod_vars = c("BFIWs", "Tmean8110Ws", "Precip8110Ws", "n_farm_inputs", "n_dev_inputs", "p_farm_inputs", "lake_dep", "lakemorpho_fetch", "AG_ECO3"), interact = "n_farm_inputs * AG_ECO3")

cyano4 <- run_cyano(mod_vars = c("BFIWs", "Tmean8110Ws", "Precip8110Ws", "n_farm_inputs", "n_dev_inputs", "p_farm_inputs", "lake_dep", "lakemorpho_fetch", "AG_ECO3"), interact = "p_farm_inputs * AG_ECO3")

cyano5 <- run_cyano(mod_vars = c("BFIWs", "Tmean8110Ws", "Precip8110Ws", "n_farm_inputs", "n_dev_inputs", "p_farm_inputs", "lake_dep", "lakemorpho_fetch", "AG_ECO3"), interact = "n_dev_inputs * AG_ECO3")

# Interact with lake depth instead of ecoregion
cyano6 <- run_cyano(mod_vars = c("BFIWs", "Tmean8110Ws", "Precip8110Ws", "n_farm_inputs", "n_dev_inputs", "p_farm_inputs", "lake_dep", "lakemorpho_fetch"), interact = "n_farm_inputs * lake_dep")

cyano7 <- run_cyano(mod_vars = c("BFIWs", "Tmean8110Ws", "Precip8110Ws", "n_farm_inputs", "n_dev_inputs", "p_farm_inputs", "lake_dep", "lakemorpho_fetch"), interact = "p_farm_inputs * lake_dep")

cyano8 <- run_cyano(mod_vars = c("BFIWs", "Tmean8110Ws", "Precip8110Ws", "n_farm_inputs", "n_dev_inputs", "p_farm_inputs", "lake_dep", "lakemorpho_fetch"), interact = "n_dev_inputs * lake_dep")
...
# N developed inputs are related. P developed input also may be important. Likely variables are highly colinear.
...
null <- spglm(MICX_DET ~ 1, data = drop_na(habs, all_of(dep_var), all_of(c("p_farm_inputs", "N_Surplus", "fst_ws", "Precip_Minus_EVTWs", "lake_dep", "lakemorpho_fetch", "BFIWs", "AG_ECO3"))), spcov_type = "none", family = "binomial", local = T)

summary(micx38)
varcomp(micx11)
beepr::beep()

aug_micx <- augment(micx38)

pROC::auc(aug_micx$MICX_DET, aug_micx$.fitted)

micx38_loovc <- loocv(micx38, cv_predict = T)
...
aug_bins <- aug_micx |>
  mutate(bin = case_when(.std.resid < -2 ~ "c1",
                         ...
                         .std.resid > 0 & .std.resid <= 1 ~ "c4",
                         .std.resid > 1 & .std.resid <= 2 ~ "c5",
                         .std.resid > 2 & .std.resid <= 2.5 ~ "c6")) |>
  arrange(abs(.std.resid))
  
ggplot() + 
  geom_sf(data = conus, fill = "#e2e2e2", color = "white", size = 0.5) +
  geom_sf(data = aug_bins, aes(fill = bin), pch = 21) +
  scale_fill_manual(values = c('#7f3b08','#e08214','#fee0b6','#d8daeb','#b2abd2','#542788','#2d004b'),
                    name = " ", 
                    breaks = c(paste0("c", 1:6)),
                    labels = (c("< -2", "-2 - -1","-1 - 0","0 - 1","1 - 2","2 - 2.5", "> 2.5"))) +
  theme_void() +
  ggtitle("Microcystin: Standardized Residuals") +
  theme(plot.title = element_text(hjust = 0.5, vjust = 1, size = 12))
ggsave("./figures/Micx_StdResidMap.png", width = 6, height = 3.5, units = "in", dpi = 600)
...
vis_fit(dep_var, micx38, file_name = "Model Fit_Micx_Det.jpg")
