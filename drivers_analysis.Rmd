...
create_formula <- function(dep_var, ind_vars, log = F, interact = NULL){
  ...
  # if(log == T){
  #   form <- paste("log10(", dep_var, ") ~ ", paste(ind_vars, collapse = " + "))
  # }
...
cyano_linear    <- splm(create_formula(dep_var, mod_vars, log = T),
...
max_val <- max(log10(cyano_pred$B_G_DENS +100), cyano_pred$preds)
...
out <- optim(0, fn = find_c, formula = B_G_DENS ~ NTL, data = dat, method = "Brent", lower = -10, upper = 10)
c_value <- exp(out$par)
print(c_value)
