...
dep_var  <- "B_G_DENS"
const <- 1000
mod_vars <- c("NTL", "PTL", "NITRATE_N", "NITRATE_DET", "AMMONIA_N", "DOC", "PH", "Precip8110Ws", "Tmean8110Ws", "D_EXCESS", "MAXDEPTH","agr_ws", "AG_ECO3")
cyano <- splm(create_formula(dep_var, mod_vars, log = T, const = const, interact = "PTL * AG_ECO3"),
...
mod_vars <- c("agr_ws", "dev_ws", "wet_ws", "BFIWs", "Tmean8110Ws", "Precip8110Ws", "lakemorpho_fetch", "nfarm_inputs_pres", "log_nfarm_inputs","ndev_inputs_pres", "log_ndev_inputs")
...
# There is *maybe* an additional interaction between n_farm_inputs and lake depth (categorical: deep/shallow). A comparison of AIC and variation captured by covariates is slightly improved with the interaction. In all these cases, the relationship between N farm inputs and cyanos is slightly negative for the western mountains/xeric ecoregion, although no highly important. 
...
# Best fit so far? Include an interaction between N farm inputs and ecoregion and *maybe* an interaction between N farm inputs and lake depth (categorical).
mod_vars <- c("BFIWs", "Tmean8110Ws", "Precip8110Ws", "n_farm_inputs", "n_dev_inputs", "p_farm_inputs", "lake_dep", "lakemorpho_fetch", "AG_ECO3")
...
fig <- ggplot(data = cyano_pred, aes(x = preds, y = log10(B_G_DENS + const))) +
  ...
  xlab("Fitted") +
  ggtitle(name)
ggsave(paste0("./figures/", "Model Fit_Cyano_No NLA Vars.jpg"), fig, height = 4, width = 5, bg = "transparent", dpi = 400, units = "in")
...
run_micx <- function(mod_vars, int = NULL){
  spglm(create_formula(dep_var = "MICX_DET", ind_vars = mod_vars, interact = NULL),
                family = "binomial",
                data = drop_na(habs, all_of(dep_var), all_of(mod_vars)),
                spcov_type = "none",
                random = ~ DSGN_CYCLE + UNIQUE_ID,
...
# Getting a lot of the following error. Seemingly only with N inputs.
# Error in .solve.checkCond(a, tol) : 
#   'a' is computationally singular, rcond(a)=1.52205e-16
...
# What happens when we replace ag cover with ag N inputs of excess ag inputs?
micx13 <- run_micx(mod_vars = c("NTL", "PH", "nfarm_inputs_pres", "log_nfarm_inputs", "ndev_inputs_pres", "log_ndev_inputs", "BFIWs", "MAXDEPTH", "EVAP_INFL", "lakemorpho_fetch", "AG_ECO3"))

# Make sense: Positive relationship - log_nfarm_inputs, ndev_inputs_presence, log_pdev_inputs
# Does not make sense: Negative relationship: log_pfarm_inputs, log_ndev_inputs
# Not related: pfarm_inputs_presence, nfarm_inputs_presence, pdev_inputs_presence
micx20 <- run_micx(mod_vars = c("pfarm_inputs_pres", "log_pfarm_inputs", "nfarm_inputs_pres", "log_nfarm_inputs", "ndev_inputs_pres", "log_ndev_inputs", "pdev_inputs_pres", "log_pdev_inputs"))

# Put it all together, but none of the nni variables came out as important.
micx21 <- run_micx(mod_vars = c("NTL", "PH", "log_nfarm_inputs", "log_pdev_inputs", "log_ndev_inputs", "BFIWs", "MAXDEPTH", "EVAP_INFL", "lakemorpho_fetch", "AG_ECO3"))

# Trying to rerun the best fit model with all variables, but now exlcuding land cover and subing in NNI vars. Still, no NNI vars emerge as important.
micx22 <- run_micx(mod_vars = c("NTL", "NITRATE_N", "PH", "EVAP_INFL", "nfarm_inputs_pres", "log_nfarm_inputs", "ndev_inputs_pres",  "pdev_inputs_pres", "log_pdev_inputs", "log_ndev_inputs", "BFIWs", "AG_ECO3", "MAXDEPTH",  "lakemorpho_fetch"))

# Try reducing the number of NNI vars. Log_P_Developed_Inputs is important, Log_N_Farm_Inputs has p = 0.15. Fetch no longer related.
micx23 <- run_micx(mod_vars = c("NTL", "NITRATE_N", "PH", "EVAP_INFL", "log_nfarm_inputs", "log_pdev_inputs", "BFIWs", "AG_ECO3", "MAXDEPTH",  "lakemorpho_fetch"))

# Removing NLA lake water data from covariates
micx24 <- run_micx(mod_vars = c("log_nfarm_inputs", "log_pdev_inputs", "BFIWs", "AG_ECO3", "MAXDEPTH",  "lakemorpho_fetch"))

# Removing fetch, replace MAXDEPTH with lakemorpho depth. 
micx25 <- run_micx(mod_vars = c("log_nfarm_inputs", "log_pdev_inputs", "BFIWs", "AG_ECO3", "lakemorpho_depth", "temp_mean_month", "precip_mean_month"))

# Trying some land cover variables. Forest area important. Agriculture and wetland cover approaching important. Developed cover not important.
micx26 <- run_micx(mod_vars = c("log_nfarm_inputs", "log_pdev_inputs", "agr_ws", "dev_ws", "fst_ws", "wet_ws", "BFIWs", "AG_ECO3", "lakemorpho_depth", "temp_mean_month"))

# Removing developed area
micx27 <- run_micx(mod_vars = c("log_nfarm_inputs", "log_pdev_inputs", "agr_ws", "fst_ws", "wet_ws", "BFIWs", "AG_ECO3", "lakemorpho_depth", "temp_mean_month"))

# Removing agriculture and wetland land cover
micx28 <- run_micx(mod_vars = c("log_nfarm_inputs", "log_pdev_inputs", "fst_ws", "BFIWs", "AG_ECO3", "lakemorpho_depth", "temp_mean_month"))

# Running with local = F
micx29 <- run_micx(mod_vars = c("n_farm_inputs", "p_dev_inputs", "fst_ws", "BFIWs", "AG_ECO3", "lakemorpho_depth", "temp_mean_month"))

summary(micx29)
varcomp(micx29)

aug_micx <- augment(micx29)

micx29_loovc <- loocv(micx29, cv_predict = T)

mapview::mapview(aug_micx, zcol = ".std.resid", at = c(-2, -1, 0, 1, 2, 2.5), layer.name = "Standardized Residual", cex = 3)



habs |>
  drop_na(MICX_DET, n_farm_inputs) |>
  slice(1:160) |>
  select(MICX_DET, n_farm_inputs) |>
