...
wsmet %>%
  select(UNIQUE_ID, DSGN_CYCLE, lakemorpho_depth, RunoffWs, Precip_Minus_EVTWs, starts_with("Pct")) %>%
  join_right(nla_id, by = c("UNIQUE_ID", "DSGN_CYCLE"))
...
wsag <- wsmet %>%
  select(COMID, DSGN_CYCLE, lakemorpho_depth, PctHay2006Ws, PctCrop2006Ws) %>%
  distinct() %>%
  mutate(PctAg2006Ws = PctHay2006Ws + PctCrop2006Ws, .keep = "unused") %>%
...
expl_var <- "NTL"
expl_var <- "Tmean8110Ws"
expl_var <- "lakemorpho_depth"
expl_var <- "PctAg2006Ws"
...
rev_biplots <- function(expl_var, file_name){
  ...
  ggplot(data = plot_data, aes(x = plot_var, y = tran_cyano)) +
    geom_point(aes(color = MICX), stroke = .7, alpha = 0.5, size = 3) +
    ...
    scale_y_continuous(trans = 'log10',
                       breaks = trans_breaks("log10", function(x) 10^x),
                       labels = trans_format("log10", math_format(10^.x))) +
    # xlab(bquote('Mean Summer Bloom Magnitude'~(CI_cyano/km^-2))) +
    # xlab('Total Nitrogen (mg N/L)') +
    # xlab('Mean Annual Temp (deg C)') +
    # xlab('Max Lake Depth (m)') +
    xlab('Watershed Agriculture Land Use (%)') +
    ylab(ylabel) +
  ...
  # Original size
  ggsave(paste0("./figures/",file_name,".png"), height = 3.5, width = 6, units = 'in', dpi = 600, bg = "transparent")
}
