...
# Duplicated rows
tmp <- wsmet_nla |>
  filter_at(vars(lakemorpho_depth, RunoffWs, Precip_Minus_EVTWs, WWTPWs, starts_with("Pct"), Precip8110Ws, Tmean8110Ws, ElevWs, SlopeWs), all_vars(is.na(.)))

length(unique(wsmet_nla$UNIQUE_ID))
length(unique(nla_id$UNIQUE_ID))

wsmet_nla |>
  select(UNIQUE_ID, VISIT_NO, DSGN_CYCLE) |>
  distinct() |>
  nrow()

tmp <- wsmet_nla |>
  group_by(UNIQUE_ID, VISIT_NO, DSGN_CYCLE) |>
  mutate(count = n()) |>
  filter(count >1)

filter(nla_id, UNIQUE_ID == "NLA_NV-10003")
filter(wsmet_nla, UNIQUE_ID == "NLA_NV-10003")
