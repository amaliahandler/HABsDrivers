...
nla <- all_nla %>%
  ...
  left_join(nla_lakecat[,c("UNIQUE_ID", "COMID", "IN_LAKECAT")], by = "UNIQUE_ID")
...
nla_2 <- all_nla %>%
  select(SITE_ID, VISIT_NO, UNIQUE_ID, DSGN_CYCLE, DATE_COL, 
         TEMPERATURE, STRATIFIED, AMMONIA_N, DOC, NTL, PTL, TURB, 
         NITRATE_N, PH, CHLA_RESULT, MICX, CYLSPER, B_G_DENS) %>%
  left_join(unique_int_nla, by = "UNIQUE_ID")

# How many are not in Lakecat?
nla %>%
  filter(is.na(COMID)) %>%
  select(SITE_ID, UNIQUE_ID, DSGN_CYCLE, COMID, IN_LAKECAT)

nla %>%
  filter(is.na(COMID)) %>%
  select(SITE_ID, UNIQUE_ID, DSGN_CYCLE, COMID)
