...
# Maybe use the topmost measurement for temperature and pH (in 2007)
# There are often two measurements of the temperature at 0 m depth, will use the first of these

# There are 3 lakes that have no measurements collected
sfc17 <- p17 %>%
  group_by(UID) %>%
  summarise(min_dep = min(DEPTH, na.rm = T),
            RESULT = TEMPERATURE[match(min_dep, min_dep)]) %>%
  select(UID, RESULT) %>%
  inner_join(nla_id, by = "UID") %>%
