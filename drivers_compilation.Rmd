...
Compiling data from the 2007, 2012, and 2017 NLA and data associated with the watersheds for each of these lakes from LakeCat and National Nutrient Inventor, in addition to NLA derived data.
...
all_nla <- read.csv("NLA2007-2012-2017_LakeData_forHABs_28Sept2023.csv")
...
# Define needed columns (will deal with missing pH values below)
nla_comid <- all_nla %>%
  select(SITE_ID, VISIT_NO, UNIQUE_ID, DSGN_CYCLE, DATE_COL, LAT_DD83, LON_DD83,  
         ...
         TURB, NITRATE_N, PH, CHLA_RESULT, MICX, CYLSPER, B_G_DENS, BG_BIOVOL,
         PHYT_BIOVOL) %>%
...
# For NITRATE, values of 0.005 in 2007 should be recoded as NA. This seems to be what was reported for anything below the RL of 0.02.
nla_long <- nla_all_coms %>%
  ...
  mutate(NITRATE_N = case_when(DSGN_CYCLE == 2007 & NITRATE_N == 0.005 ~ NA,
                               TRUE ~ NITRATE_N)) |>
  pivot_longer(TEMPERATURE:PHYT_BIOVOL, names_to = "ANALYTE", values_to = "RESULT")
...
nla_long <- nla_long %>%
  ...
  mutate(UNITS = case_when(ANALYTE == "TEMPERATURE" ~ "DEG_C",
                           ...
                           ANALYTE == "DO_SURF" ~ "DEG_C",
                           ANALYTE %in% c("PTL", "MICX", "CYLSPER") ~ "UG/L",
                           ANALYTE == "TURB" ~ "NTU",
                           ANALYTE == "B_G_DENS" ~ "CELLS/ML",
                           ANALYTE %in% c("BG_BIOVOL", "PHYT_BIOVOL") ~ "UM3/ML",
                           ANALYTE == "PH" ~ "STD_UNITS",
                           ANALYTE %in% c("AMMONIA_N", "DOC", "NTL", "NITRATE_N", "CHLA_RESULT") ~ "MG/L"))

# Performing checks for missing data

# Lots of Nitrate data missing. Looks like everything below the reporting limit was listed as 0.005 mg/L in 2007 (RL = 0.02). Note these observations are replace with NA above. In 2012 (RL = 0.02) and 2017 (RL = 0.0025) ND for nitrate is reported as NA. 
nla_long %>%
...
missing_lakes <- rename(missing_lakes, geometry = geom)
...
extractor <- function(x, year){
  x |> 
    select(SITE_ID, VISIT_NO, N_Total_Deposition_2002:Recovered_P_2012) |> # contains(c("Total_Inputs", "Surplus"))) |>
...
nni_mean <- nni0712 |>
  ...
  pivot_wider(id_cols = UNIQUE_ID,
              ...
              values_from = mean020712)


# Old code, for when I just wanted to total inputs
# nni_mean <- nni0712 |>
...
```{r}
# Read in data
nla_ecor <- readr::read_csv("NLA_UniqueIDs_HABsDrivers_Ecoregions.csv")
