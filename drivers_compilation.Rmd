...
tmp <- nni0712 |>
  ...
  pivot_longer(cols = N_Total_Deposition_2002:P_Recovered_P_2012,
               ...
               # Stuck here, trying to figure out how to breakup the string after the N/P_, then anything in the middle, then the year after the final underscore.
               # Now stuck with leading and trailing underscores
               names_sep = c(1, -4),
               values_to = "amount") |>
  ...
  mutate(type = case_when(type == "Livestock.Waste" ~ "livestock_Waste",
                          ...
                          TRUE ~ type)) |>
  # What is Low P and High_P? 2007 seems to be missing high_P. Need to check that all three years are present for each data type
  filter(VISIT_NO == 1) |>
  select(SITE_ID, nutrient, type, year, amount) |>
  group_by(SITE_ID, nutrient, type) |>
  summarise(count = n()) |>
  filter(count < 3)
  pull(type) |>
  unique()
  ...
  select(UNIQUE_ID, nutrient:amount)
